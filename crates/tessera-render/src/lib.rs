...
/// result to `tessera_wayland::Server::new_with_render_caps`.
...
struct OrderedSurfaceOptions<'a> {
    ...
    windows: Option<&'a [tessera_desktop::window::Window]>,
    window_filter: Option<&'a HashSet<tessera_desktop::window::WindowId>>,
...
pub struct WorkspaceSurfaceLayer<'a> {
    windows: &'a [tessera_desktop::window::Window],
    window_filter: &'a HashSet<tessera_desktop::window::WindowId>,
}
...
fn ordered_window_surfaces(
    ...
    owner: impl Fn(OrderedSurfaceSource) -> Option<tessera_desktop::window::WindowId>,
    filter: Option<&HashSet<tessera_desktop::window::WindowId>>,
) -> Vec<(
...
pub fn window_casts_shadow(window: &tessera_desktop::window::Window) -> bool {
    window_casts_resize_shadow(window)
}

fn window_casts_resize_shadow(window: &tessera_desktop::window::Window) -> bool {
    !window.read_only
        && !window.minimized
...
impl Renderer {
        ...
        for (source, window_id, last) in surfaces {
                ...
                && let Some(window) = windows.as_ref().and_then(|windows| windows.get(&window_id))
            {
                match options.shadow_style {
                    tessera_desktop::window::WindowShadowStyle::None => {}
                    ...
                    tessera_desktop::window::WindowShadowStyle::Soft => {
                        if window_casts_resize_shadow(window)
                            ...
                            })
                        {
                            ...
                            let geom = flux::sys::flux_geometry {
                                ...
                                _pad: [0; 3],
                                stroke: flux::sys::flux_stroke_style {
                                    width: 0.0,
                                    ..Default::default()
                                },
    ...
    pub fn draw_closing_frames(
        ...
        for ghost in ghosts {
            ...
            let style = flux::ImageStyle {
                tint: flux::rgba(255, 255, 255, alpha),
                blend: flux::BlendMode::SrcOver,
            };
            ...
            if let Some(entry) = self.cache.get(&f.id) {
                ...
                if let Some(map) = map {
                    ...
                    if let Some(style) = mapped_style {
                        ...
                        let image_style = flux::ImageStyle {
                            tint: flux::rgba(brightness, brightness, brightness, alpha),
                            blend: flux::BlendMode::SrcOver,
                        };
                        let clip = style.rounded_clip;
                        canvas.draw_image_clipped_rrect_with_style(
                            img,
...
mod tests {
    ...
    fn resize_shadow_follows_direct_resize_eligibility() {
        ...
        window.layout_role = tessera_desktop::layout::LayoutRole::Tiled;
        assert!(!window_casts_resize_shadow(&window));
    }
