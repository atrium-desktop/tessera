...
use tessera_model::input::{KeyChar, XKB_KEY_Escape, XKB_KEY_ISO_Left_Tab,
    XKB_KEY_Left, XKB_KEY_Return, XKB_KEY_Right, XKB_KEY_Tab, XKB_KEY_Up, XKB_KEY_Down,
    key_action};
...
impl Pivot {
    ...
    fn app_icon_by_id(&self, app_id: &str) -> Option<*mut c_void> {
        ...
        get(app_id.strip_suffix(".desktop").unwrap_or(app_id))
            .or_else(|| self.icons.default_icon())
    }
    ...
    pub fn build_search_items(
        ...
        if let Some(res) = eval_math(q) {
            items.push(PivotItem {
                ...
                icon: ItemIcon::Math,
                is_running: false,
                action: PivotAction::CopyCalculation(res),
            });
        }
        ...
        for cmd in SystemCmd::ALL {
            if cmd.matches(q) {
                items.push(PivotItem {
                    ...
                    icon: ItemIcon::System(cmd.icon()),
                    is_running: false,
                    action: PivotAction::SystemCommand(cmd),
                });
            }
        }
        ...
        for window in windows {
            ...
            if title_match || app_match {
                ...
                let icon_ptr = self.app_icon_by_id(app_id);

                items.push(PivotItem {
                    title: win_title,
                    subtitle: Some(win_sub),
                    icon: ItemIcon::App(icon_ptr),
                    is_running: true,
                    action: PivotAction::FocusWindow(window.id),
                });
            }
        }

        // 4. Applications (Level 1)
        for &app_index in self.brain.filtered().iter() {
            let entry = &self.brain.apps()[app_index];
            let subtitle = entry
                .generic_name
                .clone()
                .or_else(|| entry.comment.clone());
            let is_running = self.brain.is_running(app_index);
            items.push(PivotItem {
                title: entry.name.clone(),
                subtitle,
                icon: ItemIcon::App(self.entry_icon(entry)),
                is_running,
                action: PivotAction::LaunchApp(app_index),
            });
        }
        ...
        if q.len() >= 2 {
            items.push(PivotItem {
                ...
                icon: ItemIcon::Agent,
                is_running: false,
                action: PivotAction::AskAgent(q.to_string()),
            });
        }
    ...
    fn panel_rect(&self, display: (f32, f32), content_count: usize, progress: f32) -> Rect {
        ...
        let content_height = if query_empty {
            ...
            let rows = ((count + BROWSE_COLS - 1) / BROWSE_COLS).clamp(1, 3);
    ...
    fn key_char(&mut self, key: &KeyChar, out: &mut ChromeEvents) {
        ...
        match key.keysym {
            ...
            XKB_KEY_Tab | XKB_KEY_ISO_Left_Tab => {
                ...
                if self.brain.query().is_empty() {
                    ...
                    let cur_pos = cats.iter().position(|&c| c == self.active_category).unwrap_or(0);
        ...
        match key.keysym {
            ...
            XKB_KEY_Return => {
                ...
                let items = self.build_search_items(self.brain.query(), &dummy_windows, &Localizer::from_env());
                if let Some(item) = items.get(self.search_selection.min(items.len().saturating_sub(1))) {
    ...
    fn render(
        ...
        if !query_empty {
            ...
            if search_items.is_empty() {
                ...
                let selection_clamped = self.search_selection.min(search_items.len().saturating_sub(1));
                                ...
                                |frame| {
                                    ...
                                    if selected {
                                        frame.row_ex(
                                            ...
                                            |frame| {
                                                frame.label_compact_sized(action_badge, type_scale.caption);
                                            },
                                        );
                                    }
                                },
...
mod tests {
    ...
    fn unified_search_includes_math_commands_and_agent() {
        ...
        assert!(cmd_items.iter().any(|i| matches!(i.action, PivotAction::SystemCommand(SystemCmd::Lock))));

        // 3. Agent fallback
        let agent_items = pivot.build_search_items("how to write a rust macro", &[], &i18n);
        assert!(agent_items.iter().any(|i| matches!(i.action, PivotAction::AskAgent(_))));
        ...
        assert!(win_items.iter().any(|i| matches!(i.action, PivotAction::FocusWindow(WindowId(42)))));
