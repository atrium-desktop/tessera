//! Lightweight, zero-dependency safe inline arithmetic evaluator for Pivot.
...
pub fn eval_math(expr: &str) -> Option<String> {
    ...
    if tokens.len() <= 2 && tokens.iter().any(|t| matches!(t, Token::Number(_))) && !tokens.iter().any(|t| matches!(t, Token::Multiply | Token::Divide | Token::Power | Token::Modulo)) {
        return None;
    }
    ...
    if (val.fract()).abs() < 1e-9 && val.abs() < 1e15 {
        ...
        Some(format!("{:.4}", val).trim_end_matches('0').trim_end_matches('.').to_string())
    }
}
