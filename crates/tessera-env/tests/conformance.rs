...
//!   `tessera-env` (the seam exists for all of them), verified
...
use tessera_env::{CleanupFixture, RuntimeDirError};
...
fn fixtures_run_in_reverse_registration_order_and_drain() {
    ...
    tessera_env::register_cleanup_fixture(Box::new(OrderProbe {
        log: log.clone(),
        name: "first",
    }));
    tessera_env::register_cleanup_fixture(Box::new(OrderProbe {
    ...
    tessera_env::run_cleanup_fixtures();
    assert_eq!(
        *log.lock().unwrap(),
...
fn fixtures_are_idempotent_through_the_graceful_path_only_once() {
    ...
    tessera_env::register_cleanup_fixture(Box::new(CountingFixture(counter.clone())));
    tessera_env::run_cleanup_fixtures();
    tessera_env::run_cleanup_fixtures();
...
fn every_binary_entry_point_declares_a_bootstrap_dependency() {
    ...
    let binaries_requiring_seam = [
        "crates/tessera/Cargo.toml",
        "crates/tessera-lock/Cargo.toml",
        "crates/tessera-idle/Cargo.toml",
    ];
    ...
    for manifest in binaries_requiring_seam {
        ...
        assert!(
            text.contains("tessera-env.workspace = true"),
            "{manifest} declares a process entry point but does not depend on tessera-env"
        );
    }
}
