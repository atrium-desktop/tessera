...
pub fn assign_slots(
    area: Rect,
    windows: &[(tessera_desktop::window::WindowId, Rect)],
) -> Vec<(tessera_desktop::window::WindowId, Rect)> {
...
mod tests {
    ...
    fn assign_slots_is_a_deterministic_permutation() {
        let area = Rect::new(0, 0, 1280, 800);
        let id = |n: u64| tessera_desktop::window::WindowId(n);
        let windows: Vec<(tessera_desktop::window::WindowId, Rect)> = (0..6)
            .map(|n| {
                (
                    id(n),
