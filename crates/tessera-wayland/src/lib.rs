...
use tessera_authority::interaction_domain::AuthorityTransfer;
use tessera_authority::interaction_domain::HUMAN_INTERACTION_DOMAIN;
use tessera_authority::interaction_domain::HUMAN_PRINCIPAL;
use tessera_authority::interaction_domain::HUMAN_SEAT;
use tessera_authority::interaction_domain::InteractionDomainBundle;
use tessera_authority::interaction_domain::InteractionDomainError;
use tessera_authority::interaction_domain::InteractionDomainId;
use tessera_authority::interaction_domain::InteractionDomainModel;
use tessera_authority::interaction_domain::InteractionDomainMutation;
use tessera_authority::interaction_domain::InteractionDomainMutationResult;
use tessera_authority::interaction_domain::InteractionDomainRevocation;
use tessera_authority::interaction_domain::InteractionDomainSnapshot;
use tessera_authority::interaction_domain::InteractionDomainTransactionReceipt;
use tessera_authority::interaction_domain::InteractionPrincipalId;
use tessera_authority::interaction_domain::PresentationTarget;
...
pub struct SurfaceRec {
    ...
    pub window: tessera_desktop::window::Window,
...
impl SurfaceRec {
    fn new(resource: *mut ffi::wl_resource) -> SurfaceRec {
        SurfaceRec {
            ...
            window: tessera_desktop::window::Window::default(),
...
fn resize_cursor_shape(edges: tessera_desktop::window::ResizeEdges) -> u32 {
    use tessera_desktop::window::ResizeEdges;
    match (
        edges.has_top(),
        edges.has_bottom(),
        edges.has_left(),
...
fn resolve_layout_role(
    ...
    _is_transient: bool,
    rule_role: Option<tessera_desktop::layout::LayoutRole>,
) -> tessera_desktop::layout::LayoutRole {
    rule_role.unwrap_or(tessera_desktop::layout::LayoutRole::Floating)
}
...
pub(crate) struct OutputGlobal {
    state: *mut State,
    info: tessera_desktop::output::OutputInfo,
...
///
/// The authority model in `tessera-authority` owns durable identities and policy.
...
pub(crate) struct SeatRuntime {
    ...
    interactive: Option<tessera_desktop::window::Interactive>,
    ...
    synthetic_target: Option<tessera_desktop::window::WindowId>,
}
...
struct ClientDestroyRecord {
    ...
    id: tessera_authority::interaction_domain::ClientId,
}
...
struct WindowSwitcherSession {
    order: Vec<tessera_desktop::window::WindowId>,
    selected: usize,
    last_forward: bool,
}
...
pub struct WorkspaceSlideLayerPresentation {
    pub windows: Vec<tessera_desktop::window::WindowId>,
    pub offset_x: f32,
}
...
impl WorkspaceSlide {
    ...
    fn offset_at(
        ...
        let progress =
            tessera_desktop::transition::ease_out_cubic(elapsed as f32 / self.duration_ms as f32);
...
pub(crate) struct State {
    ...
    clients: std::collections::HashMap<usize, tessera_authority::interaction_domain::ClientId>,
    ...
    client_process_ids:
        std::collections::HashMap<tessera_authority::interaction_domain::ClientId, u32>,
    ...
    client_initial_interaction_domains: std::collections::HashMap<
        tessera_authority::interaction_domain::ClientId,
        InteractionDomainId,
    >,
    ...
    damaged_windows: std::collections::BTreeSet<tessera_desktop::window::WindowId>,
    ...
    pub(crate) attention_pulses: std::collections::HashMap<tessera_desktop::window::WindowId, u64>,
    ...
    layout_params: tessera_desktop::layout::LayoutParams,
    ...
    decoration_policy: tessera_desktop::window::DecorationPolicy,
    ...
    window_rules: Vec<tessera_desktop::window_rule::WindowRule>,
    ...
    pub(crate) output_geometry: tessera_desktop::output::OutputGeometry,
    ...
    output_infos: Vec<tessera_desktop::output::OutputInfo>,
    ...
    /// `set_outputs`.
    output_policies: std::collections::HashMap<String, tessera_desktop::output::OutputPolicy>,
    ...
    pub(crate) color_pipeline: tessera_desktop::output::ColorPipeline,
    ...
    workspaces: tessera_desktop::workspace::WorkspaceModel,
    /// Focused output for new surfaces and workspace commands.
    output: tessera_desktop::workspace::OutputId,
