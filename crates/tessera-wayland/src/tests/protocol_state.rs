...
fn explicit_geometry_size_respects_client_hints() {
    let hints = tessera_desktop::window::SizeHints {
        min_w: 320,
        min_h: 200,
        max_w: 1920,
        max_h: 1080,
...
fn layout_role_resolution_prefers_rule_then_transient_then_workspace() {
    use tessera_desktop::layout::LayoutRole;
...
fn fullscreen_reconfigure_covers_the_output_and_restores_the_floating_rect() {
    let mut state = State::new(std::ptr::null_mut());
    state.output_geometry = tessera_desktop::output::OutputGeometry {
...
fn set_toplevel_fullscreen_guards_follow_window_authority() {
    ...
    let human_window = tessera_desktop::window::WindowId(9001);
    ...
    let agent_window = tessera_desktop::window::WindowId(9002);
    ...
    let unknown = tessera_desktop::window::WindowId(4242);
...
fn enroll_windows_on_current_workspace(
    state: &mut State,
    windows: &[tessera_desktop::window::WindowId],
) {
    let client = state.authority.register_client(None);
...
fn unparented_toplevel_unmap_falls_back_to_the_most_recent_window() {
    let mut state = State::new(std::ptr::null_mut());
    let older = tessera_desktop::window::WindowId(1);
    let newer = tessera_desktop::window::WindowId(2);
    let dialog = tessera_desktop::window::WindowId(3);
...
fn keyboard_focus_fallback_skips_minimized_hidden_and_agent_controlled_windows() {
    let mut state = State::new(std::ptr::null_mut());
    let older = tessera_desktop::window::WindowId(1);
    let minimized = tessera_desktop::window::WindowId(2);
    let hidden = tessera_desktop::window::WindowId(3);
    let agent_mirror = tessera_desktop::window::WindowId(4);
...
fn toplevel_unmap_with_focus_on_its_own_popup_still_defers_a_restoration() {
    let mut state = State::new(std::ptr::null_mut());
    let owner = tessera_desktop::window::WindowId(1);
    let other = tessera_desktop::window::WindowId(2);
...
fn deferred_restoration_is_dropped_once_focus_moves_to_a_new_toplevel() {
    let mut state = State::new(std::ptr::null_mut());
    let mut owner = mapped_toplevel_fixture(tessera_desktop::window::WindowId(1), 0x100);
    let mut popup = Box::new(SurfaceRec::new(0x200usize as *mut ffi::wl_resource));
    popup.mapped = true;
    popup.xdg_popup = 0x201usize as *mut ffi::wl_resource;
    popup.popup_parent = owner.as_mut();
    let mut fresh = mapped_toplevel_fixture(tessera_desktop::window::WindowId(2), 0x300);
...
fn minimize_flight_targets_the_dock_icon_per_style() {
    ...
    state.minimize_animation = tessera_desktop::dock::MinimizeAnimationStyle::Genie;
    assert_eq!(minimize_flight_target(&state, window, window_rect), icon);
    ...
    assert_eq!(
        transition.easing,
        tessera_desktop::transition::Easing::EaseInCubic
    );
    assert_eq!(
        transition.effect,
...
fn modal_topology_resolves_deep_descendant_chains_and_topmost_leaf() {
    let mut state = State::new(std::ptr::null_mut());
    let root_id = tessera_desktop::window::WindowId(1);
    let child_id = tessera_desktop::window::WindowId(2);
    let leaf_id = tessera_desktop::window::WindowId(3);
    let sibling_id = tessera_desktop::window::WindowId(4);
...
fn shifting_parent_toplevel_moves_transient_children_synchronously() {
    let mut state = State::new(std::ptr::null_mut());
    let root_id = tessera_desktop::window::WindowId(10);
    let child_id = tessera_desktop::window::WindowId(11);
...
fn dnd_v3_action_negotiation_acceptance_criteria() {
    ...
    assert!(
        accepted_v3,
        "v3 DnD with negotiated action must be accepted even without explicit accept() call"
    );
    ...
    assert!(
        !accepted_rejected,
        "v3 DnD with action NONE and no accept must be rejected"
    );
...
fn dnd_origin_validation_matches_toplevel_tree_for_subsurface_clicks() {
    let mut state = State::new(std::ptr::null_mut());
    let root_id = tessera_desktop::window::WindowId(20);
    ...
    let mut unrelated = mapped_toplevel_fixture(tessera_desktop::window::WindowId(21), 0x3000);
    ...
    let matches_origin =
        |origin: *mut ffi::wl_resource, grab_surface: *mut ffi::wl_resource, st: &State| -> bool {
...
fn touch_dnd_origin_validation_accepts_touch_serial_and_surface() {
    let mut state = State::new(std::ptr::null_mut());
    let root_id = tessera_desktop::window::WindowId(30);
    ...
    let validate_drag_origin = |serial: u32,
                                origin: *mut ffi::wl_resource,
                                st: &State|
     -> Option<crate::DragOriginDevice> {
        ...
        if is_pointer_grab {
            ...
            Some(crate::DragOriginDevice::Touch {
                id: st.touch_grab_id,
            })
        }
    };
...
fn touch_dnd_full_lifecycle_state_machine() {
    let mut state = State::new(std::ptr::null_mut());
    let root_id = tessera_desktop::window::WindowId(40);
