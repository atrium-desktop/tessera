...
fn interaction_domain_window_registration_schedules_layout_and_damage_observation() {
    ...
    let window = tessera_desktop::window::WindowId(4242);
    server
        .state
...
fn single_seat_client_route_follows_atomic_group_authority() {
    ...
    let group = state
        ...
        .create_interaction_group(
            client,
            &[tessera_desktop::window::WindowId(1)],
...
fn observers_are_surface_output_members_without_receiving_control() {
    ...
    let agent = state
        .authority
        .create_agent_interaction_domain("agent", SeatCapabilities::POINTER_KEYBOARD);
    let window = tessera_desktop::window::WindowId(7);
    let client = state.authority.register_client(None);
    let group = state
        .authority
...
fn physical_observer_mirror_blocks_click_through_without_taking_focus() {
    ...
    let agent = state
        .authority
        .create_agent_interaction_domain("agent", SeatCapabilities::POINTER_KEYBOARD);
    let bottom_window = tessera_desktop::window::WindowId(1);
    let mirror_window = tessera_desktop::window::WindowId(2);
    ...
    let make_surface =
        |window: tessera_desktop::window::WindowId, resource: usize| -> Box<SurfaceRec> {
...
fn switcher_preview_does_not_restack_and_stationary_rehit_tracks_the_commit() {
    let mut state = State::new(std::ptr::null_mut());
    let selected_window = tessera_desktop::window::WindowId(1);
    let old_top_window = tessera_desktop::window::WindowId(2);
    let client = state.authority.register_client(None);
    state
        .authority
        .create_interaction_group(
...
fn window_snapshot_drops_an_unmapped_toplevel() {
    let mut state = State::new(std::ptr::null_mut());
    let window = tessera_desktop::window::WindowId(77);
    let client = state.authority.register_client(None);
    state
        .authority
        .create_interaction_group(client, &[window], HUMAN_INTERACTION_DOMAIN)
...
fn window_signature_memo_reuses_within_a_millisecond_and_invalidates_on_change() {
    ...
    // never the semantics.
    let mut state = State::new(std::ptr::null_mut());
    let window = tessera_desktop::window::WindowId(78);
    let client = state.authority.register_client(None);
    state
        .authority
        .create_interaction_group(client, &[window], HUMAN_INTERACTION_DOMAIN)
...
fn always_on_top_windows_stay_above_raised_normal_windows() {
    let mut state = State::new(std::ptr::null_mut());
    let window_a = tessera_desktop::window::WindowId(1);
    let window_b = tessera_desktop::window::WindowId(2);
    let client = state.authority.register_client(None);
    state
        .authority
...
fn restack_keeps_unfocused_newcomers_below_the_always_on_top_band() {
    let mut state = State::new(std::ptr::null_mut());
    let window_a = tessera_desktop::window::WindowId(1);
    let window_c = tessera_desktop::window::WindowId(3);
    let client = state.authority.register_client(None);
    state
        .authority
...
fn client_surface_order_keeps_each_window_tree_occluded_as_a_unit() {
    let mut state = State::new(std::ptr::null_mut());
    let background_window = tessera_desktop::window::WindowId(10);
    let foreground_window = tessera_desktop::window::WindowId(20);
    let client = state.authority.register_client(None);
    state
        .authority
        .create_interaction_group(
