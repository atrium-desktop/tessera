...
impl State {
    ...
    pub(super) fn note_open_transition(&mut self, rec: *mut SurfaceRec) {
        ...
        unsafe {
            ...
            (*rec).window.transition = Some(tessera_desktop::transition::WindowTransition::open(
                from, now,
            ));
        }
    }
    ...
    pub(super) unsafe fn note_close_transition(&mut self, rec: *mut SurfaceRec) {
        ...
        unsafe {
            ...
            let now = self.now_ms();
            let transition = tessera_desktop::transition::WindowTransition::close(rect, now);
    ...
    pub(super) fn new(display: *mut ffi::wl_display) -> State {
        let mut workspaces = tessera_desktop::workspace::WorkspaceModel::new();
        ...
        State {
            ...
            output_geometry: tessera_desktop::output::OutputGeometry::default(),
            ...
            color_pipeline: tessera_desktop::output::ColorPipeline::default(),
    ...
    pub(super) fn register_window(
        &mut self,
        client: tessera_authority::interaction_domain::ClientId,
        window: tessera_desktop::window::WindowId,
        ...
        unsafe {
            ...
            if !was_multi_seat && bound.len() > 1 {
                let _ = self.authority.set_client_multi_seat(
                    id,
                    tessera_authority::interaction_domain::MultiSeatSupport::Supported,
                );
    ...
    pub(super) fn client_routed_seat(
        ...
        if self.authority.client(client_id).is_some_and(|client| {
            client.multi_seat == tessera_authority::interaction_domain::MultiSeatSupport::Supported
        }) {
            return advertised;
        }
    ...
    pub(super) fn alloc_window_id(&mut self) -> tessera_desktop::window::WindowId {
        let id = self.next_window_id;
        self.next_window_id += 1;
        tessera_desktop::window::WindowId(id)
    }
