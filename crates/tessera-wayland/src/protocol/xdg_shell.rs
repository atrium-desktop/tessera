...
unsafe extern "C" fn xdg_surface_get_toplevel(
    ...
    unsafe {
        ...
        let window_id = if !(*rec).state.is_null() {
            ...
            tessera_desktop::window::WindowId(0)
        };
        (*rec).window = tessera_desktop::window::Window::new(window_id);
...
pub(crate) fn minimize_transition(
    ...
    match state.minimize_targets.get(&window_id) {
        Some(icon) => tessera_desktop::transition::WindowTransition::minimize(
            from,
            now,
            state.minimize_animation,
            *icon,
        ),
        None => tessera_desktop::transition::WindowTransition::new(from, now),
    }
}
...
pub(crate) unsafe fn begin_toplevel_interactive_move(rec: *mut SurfaceRec, origin: (f32, f32)) {
    unsafe {
        ...
        let was_tiled = (*rec).window.layout_role != tessera_desktop::layout::LayoutRole::Floating;
        ...
        (*state_ptr).interactive = Some(tessera_desktop::window::Interactive::Move {
...
unsafe extern "C" fn toplevel_resize(
    ...
    unsafe {
        ...
        let edges = tessera_desktop::window::ResizeEdges(edges);
        if edges.is_none() {
            return;
        }
        (*rec).window.layout_role = tessera_desktop::layout::LayoutRole::Floating;
        ...
        reconfigure_with_state(rec);
        (*state_ptr).interactive = Some(tessera_desktop::window::Interactive::Resize {
            window_id: (*rec).window.id,
            edges,
...
impl Server {
    pub(crate) fn apply_interactive_motion(&mut self, x: f32, y: f32) -> bool {
        ...
        match interactive {
            tessera_desktop::window::Interactive::Move {
                start_position,
                origin,
                ..
            } => {
