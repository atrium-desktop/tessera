...
pub(crate) unsafe fn reconcile_output_globals(
    state: &mut State,
    outputs: &[tessera_desktop::output::OutputInfo],
) {
    unsafe {
        for global in &mut state.output_globals {
...
) -> Option<tessera_desktop::output::OutputInfo> {
    unsafe {
        if resource.is_null() {
            return None;
        }
...
unsafe fn output_params(
    global: *mut OutputGlobal,
) -> (tessera_desktop::output::OutputMode, i32, i32) {
    unsafe {
        let mut mode = tessera_desktop::output::OutputMode {
            width: 1280,
            height: 720,
            refresh_mhz: 60000,
        };
