...
/// uses; duplicated here so tessera-wayland stays independent of tessera-platform.
...
pub(crate) fn decode_viewport_source(
    ...
    w: i32,
    h: i32,
) -> Result<Option<tessera_primitives::Rect>, ()> {
    if [x, y, w, h]
        .into_iter()
    ...
    Ok(Some(tessera_primitives::Rect::new(
...
unsafe extern "C" fn viewport_set_destination(
    ...
    unsafe {
        ...
        (*rec).pending_viewport_dst = Some(Some(tessera_primitives::Size { w, h }));
    }
}
