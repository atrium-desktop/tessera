...
impl Server {
    ...
    pub fn prepare_interaction_domain_portal(
        ...
        if record.state != tessera_authority::interaction_domain::InteractionDomainState::Active {
            return Err(
        ...
        let base = self
            .runtime_dir
            .as_deref()
            .map(|directory| directory.join("tessera-portals"))
            .ok_or_else(|| {
    ...
    pub(crate) fn interaction_group_output_interaction_domains(
        &self,
        group: tessera_authority::interaction_domain::InteractionGroupId,
        ...
        for mutation in mutations {
            match *mutation {
                ...
                InteractionDomainMutation::SetState {
                    interaction_domain,
                    state: tessera_authority::interaction_domain::InteractionDomainState::Active,
                } => {
    ...
    pub(crate) fn layout_virtual_interaction_domain(
        ...
        for (window, slot) in windows.into_iter().zip(slots) {
            ...
            self.state.interaction_domain_placements.insert(
                ...
                tessera_desktop::layout::grid::fit(slot, size),
            );
        }
