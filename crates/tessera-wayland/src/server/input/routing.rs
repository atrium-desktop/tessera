...
impl Server {
    ...
    pub fn client_occupies_point(&self, x: f32, y: f32) -> bool {
        !self.hit_test_focus(x, y).is_null()
            || self
                .resize_target_at(x, y, tessera_desktop::window::RESIZE_OUTER_MARGIN)
                .is_some()
    }
    ...
    pub fn windows(&self) -> Vec<tessera_desktop::window::Window> {
    ...
    pub fn render_windows(&self) -> Vec<tessera_desktop::window::Window> {
    ...
    /// applies.
    pub fn all_windows(&self) -> Vec<tessera_desktop::window::Window> {
    ...
    pub fn trigger_attention_pulse(&mut self, window_id: tessera_desktop::window::WindowId) {
        let now = self.now_ms();
        self.state.attention_pulses.insert(window_id, now);
    }

    /// Whether an attention pulse is currently active for `window_id`.
    pub fn attention_pulse_active(&self, window_id: tessera_desktop::window::WindowId) -> bool {
        let now = self.now_ms();
        self.state
            .attention_pulses
            .get(&window_id)
    ...
    pub fn human_controls_window(&self, window: tessera_desktop::window::WindowId) -> bool {
        self.state
            .authority
    ...
    pub fn close_toplevel(&mut self, surface_id: tessera_desktop::window::WindowId) {
    ...
    pub fn minimize_toplevel(&mut self, surface_id: tessera_desktop::window::WindowId) {
        if !self.human_controls_window(surface_id) {
            return;
        }
    ...
    pub fn set_toplevel_maximized(
        &mut self,
        surface_id: tessera_desktop::window::WindowId,
        maximized: bool,
    ) -> bool {
        if !self.human_controls_window(surface_id) {
            return false;
    ...
    pub fn toplevel_fullscreen(&self, surface_id: tessera_desktop::window::WindowId) -> bool {
        let rec = self.find_surface_by_window_id(surface_id);
    ...
    pub fn start_interactive_move(&mut self, surface_id: tessera_desktop::window::WindowId) {
    ...
    pub fn start_interactive_resize(
        &mut self,
        surface_id: tessera_desktop::window::WindowId,
        edges: tessera_desktop::window::ResizeEdges,
    ) {
        if self.state.interactive.is_some()
            || !self
                .state
        ...
        self.change_keyboard_focus(unsafe { (*rec).resource });
        unsafe {
            (*rec).window.layout_role = tessera_desktop::layout::LayoutRole::Floating;
        ...
        self.state.interactive = Some(tessera_desktop::window::Interactive::Resize {
    ...
    pub fn interactive(&self) -> Option<tessera_desktop::window::Interactive> {
    ...
    pub fn focus_surface_by_id(&mut self, surface_id: tessera_desktop::window::WindowId) {
    ...
    pub fn focused_toplevel_id(&self) -> Option<tessera_desktop::window::WindowId> {
    ...
    pub fn compositor_cursor_shape(&self) -> Option<u32> {
        match self.state.interactive {
            Some(tessera_desktop::window::Interactive::Move { .. }) => Some(17), // grabbing
            Some(tessera_desktop::window::Interactive::Resize { edges, .. }) => {
            ...
            None => self
                .resize_target_at(
                    ...
                    tessera_desktop::window::RESIZE_OUTER_MARGIN,
                )
