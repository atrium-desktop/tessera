...
fn is_top_edge_only(edges: tessera_desktop::window::ResizeEdges) -> bool {
...
fn stacked_resize_target<T: Copy>(
    layers: impl IntoIterator<Item = (T, tessera_desktop::window::Window, bool)>,
    x: f32,
    y: f32,
    margin: f32,
) -> Option<(T, tessera_desktop::window::ResizeEdges)> {
    let mut target = None;
    for (id, window, resizable) in layers {
        let edges = if resizable {
            window.resize_edges_at(x, y, margin)
        } else {
            tessera_desktop::window::ResizeEdges::NONE
        };
...
impl Server {
    ...
    pub(crate) fn pointer_motion(
        ...
        let focus =
            if self.state.implicit_grab_active && !self.state.implicit_grab_surface.is_null() {
                let hit = self.hit_test_focus(x, y);
                let grab_client =
                    unsafe { ffi::wl_resource_get_client(self.state.implicit_grab_surface) };
    ...
    pub(crate) fn pointer_button(&mut self, button: u32, state: tessera_primitives::input::ButtonState) {
        ...
        if !state.is_pressed() && self.state.interactive.is_some() {
            ...
            let top_border_click = match self.state.interactive {
                Some(tessera_desktop::window::Interactive::Resize {
                    window_id,
                    edges,
                    origin,
                    start_position,
    ...
    pub(crate) fn deliver_prepared_keyboard_event(
            ...
            && (!locked || self.state.allow_quit_while_locked)
        {
            keymap
                ...
                .filter(|action| {
                    !locked || matches!(action, tessera_desktop::keybind::Action::Quit)
                })
        } else {
            None
        };
    ...
    pub(crate) fn resize_target_at(
        ...
        y: f32,
        margin: f32,
    ) -> Option<(*mut SurfaceRec, tessera_desktop::window::ResizeEdges)> {
        let visible = self.visible();
    ...
    pub(crate) fn finish_interactive(&mut self) {
        ...
        if let Some(interactive_grab) = interactive {
            let window_id = match interactive_grab {
                tessera_desktop::window::Interactive::Move { window_id, .. } => window_id,
                tessera_desktop::window::Interactive::Resize { window_id, .. } => window_id,
            };
            ...
            if !rec.is_null() {
                unsafe {
                    if matches!(
                        interactive_grab,
                        tessera_desktop::window::Interactive::Resize { .. }
                    ) {
...
mod resize_tests {
    ...
    fn topmost_outer_margin_wins_over_lower_content() {
        ...
        assert_eq!(
            target,
            Some((2, tessera_desktop::window::ResizeEdges::RIGHT))
        );
    }
    ...
    fn top_border_double_click_requires_same_window_time_and_position() {
        let first = TopBorderClick {
            window_id: tessera_desktop::window::WindowId(7),
            released_at_ms: 1_000,
            position: (120.0, 40.0),
        };
        assert!(is_matching_top_border_double_click(
            first,
            tessera_desktop::window::WindowId(7),
            1_400,
            (123.0, 42.0),
        ));
        assert!(!is_matching_top_border_double_click(
            first,
            tessera_desktop::window::WindowId(8),
            1_400,
            (123.0, 42.0),
        ));
        assert!(!is_matching_top_border_double_click(
