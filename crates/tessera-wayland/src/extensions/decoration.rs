...
fn protocol_mode(policy: tessera_desktop::window::DecorationPolicy) -> u32 {
    match policy {
...
mod tests {
    ...
    fn borderless_policy_uses_server_side_protocol_ownership() {
        assert_eq!(
            protocol_mode(tessera_desktop::window::DecorationPolicy::Borderless),
            ffi::ZXDG_TOPLEVEL_DECORATION_V1_MODE_SERVER_SIDE
        );
    }
    ...
    fn client_side_policy_uses_client_side_protocol_mode() {
        assert_eq!(
            protocol_mode(tessera_desktop::window::DecorationPolicy::ClientSide),
            ffi::ZXDG_TOPLEVEL_DECORATION_V1_MODE_CLIENT_SIDE
        );
    }
}
