...
use tessera_authority::interaction_domain::InteractionDomainMutation;
...
use tessera_authority::interaction_domain::SeatCapabilities;
...
fn dispatch_command(socket: &Path, cli: Cli) -> Result<String, CliError> {
    ...
    match command {
        ...
        Cmd::Query { query } => {
            ...
            if json {
                let rows: Vec<_> = items.iter().map(|item| serde_json::json!({
                    ...
                    "kind": format!("{:?}", item.kind), "action": format!("{:?}", item.action),
                })).collect();
                serde_json::to_string_pretty(&rows).map_err(|e| CliError::Io(e.to_string()))
            } else {
                Ok(items
                    .iter()
                    .map(|item| match item.subtitle.as_deref() {
        ...
        Cmd::Overview => {
            ...
            client
                .command(tessera_protocol::Command::ToggleOverview)
                .map_err(io_err)?;
        ...
        Cmd::Quit => {
            let mut client = owner_client(socket, control_caps()).map_err(connect_err)?;
            client
                .command(tessera_protocol::Command::Quit)
                .map_err(io_err)?;
...
fn audit_error(error: tessera_audit::AuditError) -> CliError {
    CliError::Fs(error.to_string())
}

fn audit_error_json(error: serde_json::Error) -> tessera_audit::AuditError {
    tessera_audit::AuditError::Decode(error)
}

fn open_audit_store_at(
    path: &Path,
) -> Result<tessera_audit::AuditLog<tessera_audit::journal::Entry>, CliError> {
    if !path.exists() {
    ...
    tessera_audit::AuditLog::open_persistent_with_options(
        tessera_audit::DEFAULT_CAPACITY,
        path,
        tessera_audit::AuditStoreOptions::default(),
    )
    .map_err(audit_error)
}
        ...
        AuditCmd::Verify { full } => {
            ...
            if full {
                ...
                for record in &records {
                    let mut reader =
                        tessera_audit::segments::SealedSegmentReader::open(record, &segments_dir)
                            .map_err(audit_error)?;
                    reader
                        .for_each_line(|line| {
...
fn dispatch_window(
    ...
    match command.unwrap_or(WindowCmd::List) {
        ...
        WindowCmd::Focus { id } => {
            let mut client = owner_client(socket, control_caps()).map_err(connect_err)?;
            client
                .command(tessera_protocol::Command::Focus {
                    id: tessera_desktop::window::WindowId(id),
                    reveal: true,
                })
                .map_err(io_err)?;
        ...
        WindowCmd::Minimize { id } => {
            let mut client = owner_client(socket, control_caps()).map_err(connect_err)?;
            client
                .command(tessera_protocol::Command::Minimize {
                    id: tessera_desktop::window::WindowId(id),
                })
                .map_err(io_err)?;
        ...
        WindowCmd::AlwaysOnTop { id, state } => {
            ...
            client
                .command(tessera_protocol::Command::SetAlwaysOnTop {
                    id: tessera_desktop::window::WindowId(id),
                    on_top,
                })
                .map_err(io_err)?;
        ...
        WindowCmd::Fullscreen { id, state } => {
            ...
            client
                .command(tessera_protocol::Command::SetFullscreen {
                    id: tessera_desktop::window::WindowId(id),
                    fullscreen,
                })
                .map_err(io_err)?;
        ...
        WindowCmd::Close { id } => {
            let mut client = owner_client(socket, control_caps()).map_err(connect_err)?;
            client
                .command(tessera_protocol::Command::Close {
                    id: tessera_desktop::window::WindowId(id),
                })
                .map_err(io_err)?;
...
fn dispatch_workspace(
    ...
    match command.unwrap_or(WorkspaceCmd::List) {
        ...
        WorkspaceCmd::Switch {
            ...
            let mut client = owner_client(socket, control_caps()).map_err(connect_err)?;
            client
                .switch_workspace_to(tessera_desktop::workspace::WorkspaceId(id))
                .map_err(io_err)?;
            Ok(receipt(format!("switched to workspace {id}"), json))
        }
        WorkspaceCmd::Switch { target } => {
            let mut client = owner_client(socket, control_caps()).map_err(connect_err)?;
            let direction: tessera_desktop::workspace::Switch = target.into();
        ...
        WorkspaceCmd::MoveWindow { window, workspace } => {
            let mut client = owner_client(socket, control_caps()).map_err(connect_err)?;
            client
                .command(tessera_protocol::Command::MoveToWorkspace {
                    window: tessera_desktop::window::WindowId(window),
                    workspace: tessera_desktop::workspace::WorkspaceId(workspace),
                })
                .map_err(io_err)?;
            Ok(receipt(
...
fn dispatch_system(socket: &Path, command: SystemCmd, json: bool) -> Result<String, CliError> {
    ...
    let (action, acknowledgement) = match command {
        ...
        SystemCmd::Mute => (tessera_protocol::SystemAction::ToggleMute, "mute toggled"),
        SystemCmd::StepVolume { delta } => (
            tessera_protocol::SystemAction::StepVolume { delta },
            "volume step queued",
        ),
        SystemCmd::Volume { level } => (
            tessera_protocol::SystemAction::SetVolume { level },
            "volume change queued",
        ),
        SystemCmd::Brightness { level } => (
            tessera_protocol::SystemAction::SetBrightness { level },
            "brightness change queued",
        ),
        SystemCmd::Wifi { state } => (
            tessera_protocol::SystemAction::SetWifi {
                enabled: state.into(),
            },
            "Wi-Fi change queued",
        ),
        SystemCmd::Bluetooth { state } => (
            tessera_protocol::SystemAction::SetBluetooth {
                enabled: state.into(),
            },
            "Bluetooth change queued",
        ),
        SystemCmd::DoNotDisturb { state } => (
            tessera_protocol::SystemAction::SetDoNotDisturb {
                enabled: state.into(),
            },
            "Do Not Disturb change queued",
        ),
        SystemCmd::PowerMode { mode } => {
            let mode = tessera_desktop::power::PowerMode::from(mode);
            (
                tessera_protocol::SystemAction::SetPowerMode { mode },
                "power mode change queued",
            )
        }
        SystemCmd::Suspend => (tessera_protocol::SystemAction::Suspend, "system suspending"),
        SystemCmd::Reboot => (tessera_protocol::SystemAction::Reboot, "system rebooting"),
        SystemCmd::PowerOff => (
            tessera_protocol::SystemAction::PowerOff,
            "system powering off",
        ),
    };
    let mut client = owner_client(socket, control_caps()).map_err(connect_err)?;
    ...
    match action {
        PermissionsCmd::List => {
            let mut client = Client::connect_scoped(
                socket,
                query_caps(),
                tessera_protocol::LOCAL_AGENT_ADMIN_SCOPE,
            )
            .map_err(connect_err)?;
            ...
            struct PermissionsView {
                principals: Vec<tessera_protocol::AgentPrincipalInfo>,
                grants: Vec<tessera_protocol::AgentGrantInfo>,
            }
...
fn format_permissions(
    principals: &[tessera_protocol::AgentPrincipalInfo],
    grants: &[tessera_protocol::AgentGrantInfo],
) -> String {
...
fn op_names(ops: &[tessera_protocol::ActorCapability]) -> String {
    ops.iter()
        .map(|op| format!("{op:?}"))
        .collect::<Vec<_>>()
        .join(", ")
...
fn dispatch_interaction_domain(
    ...
    let mut client = Client::connect_scoped(
        socket,
        caps,
        tessera_protocol::LOCAL_INTERACTION_DOMAIN_ADMIN_SCOPE,
    )
    .map_err(connect_err)?;
    match action {
        ...
        InteractionDomainCmd::Transfer {
            ...
            let interaction_domain = validate_interaction_domain_id(interaction_domain)?;
            let window = tessera_desktop::window::WindowId(window);
            let snapshot = client.interaction_domains().map_err(io_err)?;
            let result = client
                .interaction_domain_action(InteractionDomainAction::Transact {
                    expected_revision: Some(snapshot.revision),
...
fn owner_client(socket: &Path, caps: ConnectionCapabilities) -> std::io::Result<Client> {
    Client::connect_scoped(socket, caps, tessera_protocol::LOCAL_OWNER_ADMIN_SCOPE)
}
...
fn format_windows(wins: &[tessera_desktop::window::Window]) -> String {
    if wins.is_empty() {
...
fn format_workspaces(snap: &tessera_desktop::workspace::WorkspaceSnapshot) -> String {
...
fn format_outputs(outs: &[tessera_desktop::output::OutputInfo]) -> String {
    if outs.is_empty() {
        return "no displays".into();
    }
    let mut out = String::new();
...
fn format_notifications(notifications: &[tessera_desktop::notify::Notification]) -> String {
...
fn format_system_status(status: &tessera_protocol::SystemStatus) -> String {
    let volume = status
        .volume
        .map(|level| format!("{level}%"))
        .unwrap_or_else(|| "unavailable".into());
    let network = match status.network {
        tessera_desktop::system::NetworkState::Offline => "offline",
        tessera_desktop::system::NetworkState::Wifi => "wifi",
        tessera_desktop::system::NetworkState::Wired => "wired",
    };
    let battery = status
        .battery
        .map(|battery| {
...
fn format_journal(snapshot: &tessera_protocol::JournalSnapshot) -> String {
...
fn format_mutation(mutation: &tessera_protocol::JournalMutation) -> String {
    match mutation {
        tessera_protocol::JournalMutation::AgentAuth { principal, action } => {
            let action = match action {
                tessera_protocol::AgentAuthAction::Paired => "paired".to_owned(),
                tessera_protocol::AgentAuthAction::Granted { op, persistence } => {
                    let scope = match persistence {
                        tessera_protocol::GrantPersistence::Once => "once",
                        tessera_protocol::GrantPersistence::Session => "for this session",
                        tessera_protocol::GrantPersistence::Always => "always",
                        tessera_protocol::GrantPersistence::DeniedSession => {
                            return format!(
                ...
                tessera_protocol::AgentAuthAction::Forgotten => "forgotten".to_owned(),
                tessera_protocol::AgentAuthAction::Renamed => "renamed".to_owned(),
                tessera_protocol::AgentAuthAction::CeilingChanged => "ceiling changed".to_owned(),
            };
...
/// Whether command execution needs a live session socket.
pub fn requires_session(cli: &Cli) -> bool {
    !matches!(
        cli.command.as_ref(),
        None | Some(
            Command::Run
...
/// Resolve the explicit or process-local socket before executing a typed command.
...
mod tests {
    use super::*;
    use std::str::FromStr;
    use tessera_desktop::notify::Notification;
    ...
    fn format_event_windows_and_workspace() {
        ...
        assert_eq!(
            format_event(&Event::SpaceUseChanged {
                state: tessera_desktop::window::SpaceUse::Maximized,
            }),
    ...
    fn audit_cli_status_verify_export_and_prune_round_trip() {
        ...
        let options = tessera_audit::AuditStoreOptions {
            min_free_bytes: 0,
        ...
        {
            let mut log = tessera_audit::AuditLog::<tessera_audit::journal::Entry>::open_persistent_with_options(8, &stream, options).unwrap();
            for sequence in 1..=12u64 {
                log.try_append(
                    sequence,
                    tessera_protocol::Origin::Internal,
                    tessera_protocol::JournalMutation::ScopeClaim {
                        scope: format!("scope-{sequence}"),
                    },
                    tessera_protocol::Effect::Applied,
                )
                .unwrap();
            }
        }
