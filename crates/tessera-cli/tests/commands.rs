...
//! test starts a server with a fixed handler, invokes `tessera_cli::run` with a
...
use tessera_desktop::window::WindowState;
use tessera_desktop::workspace::OutputSnapshot;
use tessera_desktop::workspace::WorkspaceEntry;
use tessera_desktop::workspace::WorkspaceId;
use tessera_desktop::workspace::WorkspaceSnapshot;
use tessera_ipc::{Handler, Server};
use tessera_protocol::{Command, InteractionDomainAction, InteractionDomainActionResult};
...
impl Handler for CtlHandler {
    ...
    fn workspaces(&self) -> WorkspaceSnapshot {
        WorkspaceSnapshot {
            outputs: vec![OutputSnapshot {
                id: tessera_desktop::workspace::OutputId(0),
    ...
    fn notifications(&self) -> Vec<tessera_desktop::notify::Notification> {
        vec![tessera_desktop::notify::Notification {
            id: 7,
    ...
    fn system_status(&self) -> tessera_protocol::SystemStatus {
        tessera_protocol::SystemStatus {
            volume: Some(42),
            muted: true,
            network: tessera_desktop::system::NetworkState::Wifi,
            battery: Some(tessera_desktop::system::BatteryStatus {
                percent: 81,
                charging: true,
            }),
            ...
            brightness: Some(73),
            do_not_disturb: true,
            ..tessera_protocol::SystemStatus::default()
        }
    }
    fn outputs(&self) -> Vec<tessera_desktop::output::OutputInfo> {
        vec![tessera_desktop::output::OutputInfo {
            connector: "nested".into(),
    ...
    fn journal_since(&self, _since: u64) -> tessera_protocol::JournalSnapshot {
        tessera_protocol::JournalSnapshot {
            entries: vec![tessera_protocol::JournalEntry {
                seq: 3,
                ts_mono_ms: 42,
                origin: tessera_protocol::Origin::Chrome,
                mutation: tessera_protocol::JournalMutation::Command {
                    cmd: tessera_protocol::AuditedCommand::Focus { id: WindowId(1) },
                },
                effect: tessera_protocol::Effect::Applied,
            }],
            oldest_seq: 1,
            latest_seq: 3,
        }
    ...
    fn resolve_scope(&self, name: &str) -> Option<tessera_protocol::Scope> {
        if name == tessera_protocol::LOCAL_OWNER_ADMIN_SCOPE {
            return Some(tessera_protocol::Scope::unscoped());
        }
        if name == tessera_protocol::LOCAL_AGENT_ADMIN_SCOPE {
            return Some(tessera_protocol::Scope {
                ops: Some(Vec::new()),
                ..tessera_protocol::Scope::default()
            });
        }
        (name == tessera_protocol::LOCAL_INTERACTION_DOMAIN_ADMIN_SCOPE).then(|| {
            tessera_protocol::Scope {
                ...
                ops: Some(vec![
                    tessera_protocol::ActorCapability::CreateInteractionDomain,
                    tessera_protocol::ActorCapability::TransactInteractionDomain,
                    tessera_protocol::ActorCapability::RevokeInteractionDomain,
                    tessera_protocol::ActorCapability::CaptureInteractionDomain,
                    tessera_protocol::ActorCapability::LaunchInInteractionDomain,
                ]),
                ask_ops: None,
            }
        })
    ...
    fn interaction_domains(
        ...
        let mut model = tessera_authority::interaction_domain::InteractionDomainModel::new();
    ...
    fn interaction_domain_action(
        ...
        match action {
            InteractionDomainAction::Create { .. } => Ok(InteractionDomainActionResult::Created {
                bundle: tessera_authority::interaction_domain::InteractionDomainBundle {
                    principal: tessera_authority::interaction_domain::InteractionPrincipalId(2),
                    interaction_domain: tessera_authority::interaction_domain::InteractionDomainId(
                        2,
                    ),
                    seat: tessera_authority::interaction_domain::SeatId(2),
                    revision: 2,
                },
            }),
            InteractionDomainAction::Transact {
                ...
                receipt:
                    tessera_authority::interaction_domain::InteractionDomainTransactionReceipt {
            ...
            InteractionDomainAction::Revoke {
                ...
                receipt: tessera_authority::interaction_domain::InteractionDomainRevocation {
                    interaction_domain,
                    fallback,
    ...
    fn agent_principals(&self) -> Vec<tessera_protocol::AgentPrincipalInfo> {
        vec![tessera_protocol::AgentPrincipalInfo {
            principal: "prin_1".into(),
            label: Some("Codex".into()),
            pregranted: vec![tessera_protocol::ActorCapability::Focus],
            gated: vec![tessera_protocol::ActorCapability::Close],
            created_at: 1,
        }]
    }

    fn agent_grants(&self, principal: Option<&str>) -> Vec<tessera_protocol::AgentGrantInfo> {
        vec![tessera_protocol::AgentGrantInfo {
            principal: "prin_1".into(),
            op: tessera_protocol::ActorCapability::Close,
            decision: tessera_protocol::AgentGrantDecision::Allow,
            granted_at: 2,
        }]
        .into_iter()
    ...
    fn register_agent(
        &self,
        label: Option<&str>,
        _pregranted: &[tessera_protocol::ActorCapability],
        _gated: &[tessera_protocol::ActorCapability],
...
fn window_domain_lists_the_fixed_window() {
    ...
    let out = tessera_cli::run(&path, &["window".into()]).unwrap();
...
fn json_flag_emits_parseable_json_for_window_domain() {
    let path = scratch();
    let _s = Server::start(&path, Arc::new(CtlHandler::new())).unwrap();
    let out = tessera_cli::run(&path, &["window".into(), "--json".into()]).unwrap();
...
fn workspace_domain_shows_display_and_workspace() {
    let path = scratch();
    let _s = Server::start(&path, Arc::new(CtlHandler::new())).unwrap();
    let out = tessera_cli::run(&path, &["workspace".into()]).unwrap();
...
fn display_domain_lists_advertised_modes_and_marks_current() {
    let path = scratch();
    let _s = Server::start(&path, Arc::new(CtlHandler::new())).unwrap();
    let out = tessera_cli::run(&path, &["display".into()]).unwrap();
...
fn notification_domain_lists_active_notifications() {
    let path = scratch();
    let _s = Server::start(&path, Arc::new(CtlHandler::new())).unwrap();
    let out = tessera_cli::run(&path, &["notification".into()]).unwrap();
...
fn system_status_command_formats_human_and_json_snapshots() {
    let path = scratch();
    let _s = Server::start(&path, Arc::new(CtlHandler::new())).unwrap();
    let out = tessera_cli::run(&path, &["system".into()]).unwrap();
    ...
    let json = tessera_cli::run(&path, &["--json".into(), "system".into()]).unwrap();
...
fn system_control_commands_send_typed_actions() {
    ...
    tessera_cli::run(&path, &["system".into(), "step-volume".into(), "-2".into()]).unwrap();
    tessera_cli::run(&path, &["system".into(), "wifi".into(), "off".into()]).unwrap();
    tessera_cli::run(
        &path,
    ...
    assert_eq!(
        ...
        &[
            Command::System {
                action: tessera_protocol::SystemAction::SetVolume { level: 55 },
            },
            Command::System {
                action: tessera_protocol::SystemAction::StepVolume { delta: -2 },
            },
            Command::System {
                action: tessera_protocol::SystemAction::SetWifi { enabled: false },
            },
            Command::System {
                action: tessera_protocol::SystemAction::SetPowerMode {
                    mode: tessera_desktop::power::PowerMode::Awake,
                },
            },
        ]
    );
...
fn interaction_domain_domain_uses_admin_scope_and_lists_agent_interaction_domain() {
    let path = scratch();
    let _s = Server::start(&path, Arc::new(CtlHandler::new())).unwrap();
    let out = tessera_cli::run(&path, &["interaction-domain".into(), "list".into()]).unwrap();
...
fn interaction_domain_create_and_transfer_use_optimistic_actions() {
    ...
    let transferred = tessera_cli::run(
        &path,
        &[
            "interaction-domain".into(),
            "transfer".into(),
    ...
    assert!(matches!(
        ...
        InteractionDomainAction::Transact {
            ...
            mutations.as_slice(),
            [tessera_authority::interaction_domain::InteractionDomainMutation::TransferWindow {
                window: WindowId(1),
                target: tessera_authority::interaction_domain::InteractionDomainId(2),
...
fn window_focus_sends_focus() {
    ...
    let out = tessera_cli::run(&path, &["window".into(), "focus".into(), "1".into()]).unwrap();
...
fn window_minimize_sends_minimize() {
    ...
    let h = Arc::new(CtlHandler::new());
    let _s = Server::start(&path, Arc::clone(&h)).unwrap();
    let out = tessera_cli::run(&path, &["window".into(), "minimize".into(), "1".into()]).unwrap();
...
fn workspace_switch_relative_sends_workspace_switch() {
    ...
    let h = Arc::new(CtlHandler::new());
    let _s = Server::start(&path, Arc::clone(&h)).unwrap();
    tessera_cli::run(&path, &["workspace".into(), "switch".into(), "next".into()]).unwrap();
    assert!(
        h.commands
            .lock()
            .unwrap()
...
fn workspace_switch_id_sends_direct_workspace_switch() {
    ...
    let h = Arc::new(CtlHandler::new());
    let _s = Server::start(&path, Arc::clone(&h)).unwrap();
    let out = tessera_cli::run(&path, &["workspace".into(), "switch".into(), "9".into()]).unwrap();
...
fn bare_and_explicit_run_invocations_select_the_compositor() {
    let bare = tessera_cli::Cli::try_parse_from(["tessera"]).unwrap();
    assert!(bare.runs_compositor());

    let explicit = tessera_cli::Cli::try_parse_from(["tessera", "run"]).unwrap();
    assert!(explicit.runs_compositor());

    let display = tessera_cli::Cli::try_parse_from(["tessera", "display"]).unwrap();
...
fn unknown_command_errors_with_usage() {
    let path = scratch();
    let _s = Server::start(&path, Arc::new(CtlHandler::new())).unwrap();
    let err = tessera_cli::run(&path, &["bogus".into()]).unwrap_err();
...
fn help_command_returns_usage() {
    let path = scratch();
    let _s = Server::start(&path, Arc::new(CtlHandler::new())).unwrap();
    let out = tessera_cli::run(&path, &["help".into()]).unwrap();
...
fn permissions_list_renders_principals_and_grants() {
    let path = scratch();
    let _s = Server::start(&path, Arc::new(CtlHandler::new())).unwrap();
    let out = tessera_cli::run(&path, &["permissions".into(), "list".into()]).unwrap();
...
fn permissions_revoke_rejects_unknown_op_names() {
    ...
    assert!(matches!(error, tessera_cli::CliError::Usage(_)));
}
