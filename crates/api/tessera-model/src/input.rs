...
pub enum SyntheticInputAction {
    ...
    /// Set the pressed/released state of one Linux `BTN_*` code, optionally moving to `position`.
    PointerButton {
    ...
    KeyPress { code: u32 },
    /// Set the pressed/released state of one Linux evdev key code against the target toplevel.
    Key { code: u32, state: ButtonState },
}
...
impl SyntheticInputAction {
    ...
    pub fn pointer_position(self) -> Option<crate::Point> {
        match self {
            ...
            Self::PointerButton { position, .. } => position,
            Self::KeyPress { .. } | Self::Key { .. } => None,
        }
    }
}

/// Convert a Linux evdev key scancode into a concise, human-readable glyph or label
/// for supervisory visualizers and feedback layers (ADR-0152).
pub fn evdev_key_name(code: u32) -> &'static str {
    match code {
        1 => "Esc",
        2 => "1",
        3 => "2",
