...
use tessera_desktop::power::IdleStageSelector;
...
impl IdlePolicy {
    pub fn validate(self) -> Result<Self, &'static str> {
        ...
        let maximum = Duration::from_secs(u64::from(
            tessera_desktop::settings::IdleSettings::MAX_TIMEOUT_SECONDS,
        ));
        if [
            self.dim_after,
            self.lock_after,
...
mod tests {
    ...
    fn refuses_timeouts_outside_the_shared_product_policy() {
        let policy = IdlePolicy {
            suspend_after: Some(Duration::from_secs(
                u64::from(tessera_desktop::settings::IdleSettings::MAX_TIMEOUT_SECONDS) + 1,
            )),
            ..IdlePolicy::default()
        };
        assert!(policy.validate().is_err());
    ...
    fn secure_mode_arms_lock_and_dim_only() {
        let policy = IdlePolicy {
            mode: tessera_desktop::power::PowerMode::Secure,
            ..IdlePolicy::default()
        };
        let stages: Vec<_> = policy.stages().map(|(stage, _)| stage).collect();
    ...
    fn modes_cannot_disable_dim_when_it_is_the_only_armed_stage() {
        ...
        for mode in tessera_desktop::power::PowerMode::ALL {
            let policy = IdlePolicy {
                mode,
                ..IdlePolicy::default()
            };
