...
/// [`tessera_desktop::power::PowerMode`] wire name (ADR-0140).
...
impl Daemon {
    ...
    fn power_outputs(&mut self, target: OutputPower) {
        ...
        let requested = tessera_protocol::ConnectionCapabilities {
            query: true,
            control: true,
            input: false,
            session: false,
        ...
        .and_then(|mut client| {
            client.apply_system_action(tessera_desktop::system::SystemAction::SetOutputPower {
        ...
        while let Ok(length) = self.control.recv(&mut message) {
            match &message[..length] {
                ...
                rest if rest.starts_with(CONTROL_MESSAGE_MODE_PREFIX) => {
                    let name: Result<tessera_desktop::power::PowerMode, String> =
                        std::str::from_utf8(&rest[CONTROL_MESSAGE_MODE_PREFIX.len()..])
                            .map_err(|_| "mode name is not UTF-8".to_owned())
                            .and_then(|name| {
                                tessera_desktop::power::PowerMode::from_name(name)
    ...
    /// stage notification.
    fn set_mode(&mut self, mode: tessera_desktop::power::PowerMode) {
        ...
        while let Some(argument) = args.next() {
            match argument.as_str() {
                ...
                "--mode" => {
                    ...
                    options.policy.mode = tessera_desktop::power::PowerMode::from_name(&name)
