...
//! (`assets/cursors/tessera-user-light`, MIT-licensed art generated by
...
static BUNDLED_THEME: Dir<'static> =
    include_dir!("$CARGO_MANIFEST_DIR/../../../assets/cursors/tessera-user-light");

static BUNDLED_DARK_THEME: Dir<'static> =
    include_dir!("$CARGO_MANIFEST_DIR/../../../assets/cursors/tessera-user-dark");

static BUNDLED_AI_THEME: Dir<'static> =
    include_dir!("$CARGO_MANIFEST_DIR/../../../assets/cursors/tessera-ai-light");

static BUNDLED_AI_DARK_THEME: Dir<'static> =
    ...
    fn svg(&self, name: &str) -> Option<Cow<'_, [u8]>> {
        match self {
            Source::Dir(dir) => std::fs::read(dir.join(format!("{name}.svg")))
                .ok()
                .map(Cow::Owned),
            Source::BundledDark => BUNDLED_DARK_THEME
                .get_file(format!("cursors/{name}.svg"))
                .map(|file| Cow::Borrowed(file.contents())),
            Source::Bundled => BUNDLED_THEME
                .get_file(format!("cursors/{name}.svg"))
                .map(|file| Cow::Borrowed(file.contents())),
            Source::BundledAiLight => BUNDLED_AI_THEME
                .get_file(format!("cursors/{name}.svg"))
                .map(|file| Cow::Borrowed(file.contents())),
            Source::BundledAiDark => BUNDLED_AI_DARK_THEME
                .get_file(format!("cursors/{name}.svg"))
                .map(|file| Cow::Borrowed(file.contents())),
        }
    }
...
impl CursorTheme {
    ...
    /// with the bundled tessera-user-light theme so a cursor exists even when nothing is
    /// installed.
    pub fn resolve(name: &str, size: u32) -> CursorTheme {
        ...
        while let Some(theme) = queue.pop() {
            ...
            match theme.as_str() {
                "tessera-user-light" | "Tessera" => {
                    chain.push(Source::Bundled);
                    continue;
                }
                "tessera-user-dark" => {
                    chain.push(Source::BundledDark);
                    continue;
                }
                "tessera-ai-light" => {
                    chain.push(Source::BundledAiLight);
                    continue;
                }
                "tessera-ai-dark" => {
                    chain.push(Source::BundledAiDark);
                    continue;
                }
                _ => {}
            }
    ...
    fn has_any_svg(&self) -> bool {
        self.chain.iter().any(|s| match s {
            ...
            Source::Bundled
            | Source::BundledDark
            | Source::BundledAiLight
            | Source::BundledAiDark => false,
        })
    }
...
impl CursorCache {
    ...
    pub fn get(&mut self, device: &flux::Device, shape: u32, scale: f32) -> Option<&LoadedCursor> {
        ...
        if !matches!(&self.theme, Some((n, s, _)) if *n == name && *s == size) {
            ...
            if matches!(
                name.as_str(),
                "tessera-user-light" | "tessera-user-dark" | "Tessera"
            ) {
                log::info!("cursor: using bundled {name:?} at {size} logical px");
            } else if theme.has_filesystem() {
                if theme.has_any_svg() {
                    log::info!(
                        "cursor: using SVG theme {name:?} at {size} logical px; missing shapes fall back to bundled tessera-user-light theme"
                    );
                } else {
                    log::warn!(
                        "cursor: theme {name:?} is installed but contains no SVG cursors \
                         ...
                         all shapes fall back to bundled tessera-user-light theme"
                    );
                }
            } else {
                log::info!(
                    "cursor: theme {name:?} not installed; using bundled tessera-user-light theme at {size} logical px"
                );
            }
...
mod tests {
    ...
    #[test]
    fn bundled_polarities_cover_shapes_without_moving_hotspots() {
        let light = CursorTheme::resolve("tessera-user-light", 24);
        let dark = CursorTheme::resolve("tessera-user-dark", 24);
        assert!(matches!(light.chain.first(), Some(Source::Bundled)));
        assert!(matches!(dark.chain.first(), Some(Source::BundledDark)));
        for shape in 1..=36 {
            for scale in [1.0, 1.5, 2.0] {
                let a = light.load_shape(shape, scale).expect("light shape");
                let b = dark.load_shape(shape, scale).expect("dark shape");
                assert_eq!((a.xhot, a.yhot), (b.xhot, b.yhot));
                assert_eq!((a.width, a.height), (b.width, b.height));
                assert_ne!(a.pixels, b.pixels, "polarity must change the pixels");
                assert!(a.pixels.chunks_exact(4).any(|pixel| pixel[3] > 0));
                assert!(
                    a.pixels
                        .chunks_exact(4)
                        .zip(b.pixels.chunks_exact(4))
                        .all(|(a, b)| a[3] == b[3]),
                    "polarity preserves coverage"
                );
            }
        }
        let legacy = CursorTheme::resolve("Tessera", 24)
            .load_shape(1, 1.0)
            .unwrap();
        assert_eq!(legacy.pixels, light.load_shape(1, 1.0).unwrap().pixels);
    }

    #[test]
    fn ai_bundled_polarities_cover_shapes() {
        let ai_light = CursorTheme::resolve("tessera-ai-light", 24);
        let ai_dark = CursorTheme::resolve("tessera-ai-dark", 24);
        assert!(matches!(ai_light.chain.first(), Some(Source::BundledAiLight)));
        assert!(matches!(ai_dark.chain.first(), Some(Source::BundledAiDark)));
        for shape in 1..=36 {
            let a = ai_light.load_shape(shape, 1.0).expect("ai light shape");
            let b = ai_dark.load_shape(shape, 1.0).expect("ai dark shape");
            assert_eq!((a.xhot, a.yhot), (b.xhot, b.yhot));
            assert_eq!((a.width, a.height), (b.width, b.height));
            assert_ne!(a.pixels, b.pixels, "polarity must change pixels");
            assert!(a.pixels.chunks_exact(4).any(|pixel| pixel[3] > 0));
        }

        // Verify that user default cursor and AI default cursor have distinct pixels (different geometry)
        let user = CursorTheme::resolve("tessera-user-light", 24)
            .load_shape(1, 1.0)
            .unwrap();
        let ai = ai_light.load_shape(1, 1.0).unwrap();
        assert_ne!(user.pixels, ai.pixels, "AI cursor must have distinct geometry from user cursor");
    }
