...
pub(super) struct FrameState {
    pub(super) input: tessera_shell::component::Input,
        ...
        .filter_map(|action| {
            let (position, kind) = match *action {
                SyntheticInputAction::PointerMove { .. } => (
                    ...
                    tessera_shell::component::AgentInputKind::PointerMove,
                ),
                SyntheticInputAction::Click { button, .. } => (
                    Some(pointer_positions.next()?),
                    tessera_shell::component::AgentInputKind::Click { button },
                ),
                SyntheticInputAction::Scroll { dx, dy, .. } => (
                    Some(pointer_positions.next()?),
                    tessera_shell::component::AgentInputKind::Scroll { dx, dy },
                ),
                ...
                SyntheticInputAction::PointerButton {
                    ...
                    tessera_shell::component::AgentInputKind::PointerButton { button, state },
                ),
            ...
            *sequence = sequence.saturating_add(1);
            Some(tessera_shell::component::AgentActivity {
                sequence: *sequence,
                interaction_domain,
...
impl CompositorRuntime<'_> {
    ...
    pub(super) fn capture_cursor_state(&self) -> CaptureCursorState {
        Self::capture_cursor_state_parts(
    ...
    pub(super) fn capture_cursor_state_parts(
        position: (f32, f32),
        display_size: (f32, f32),
        server: &tessera_wayland::Server,
        shell: &tessera_shell::Shell,
    ) -> CaptureCursorState {
        let chrome_cursor = (!server.session_locked())
            .then(|| shell.cursor_shape_at(position.0, position.1, display_size))
            .flatten();
        ...
        let hidden = owned_cursor.is_none() && server.cursor_hidden();
        CaptureCursorState {
            position,
            shape: owned_cursor.unwrap_or_else(|| server.cursor_shape().max(1)),
            hidden,
            client_surface: hidden && server.client_cursor_surface_active(),
        }
    }
                ...
                match action {
                    ...
                    GestureAction::WorkspaceSwitch => {
                        ...
                        for _ in 0..steps.unsigned_abs() {
                            let dir = if steps > 0 {
                                tessera_desktop::workspace::Switch::Next
                            } else {
                                tessera_desktop::workspace::Switch::Prev
                            };
                            apply_command_and_journal(
                                self.server,
                                &self.notif_queue,
                                &mut self.quit_requested,
                                tessera_protocol::Command::SwitchWorkspace { dir },
                                &self.ipc,
                                &self.journal,
                                ts,
                                tessera_protocol::Origin::Gesture,
                            );
                        }
                    }
                    GestureAction::WindowCycle => {
                        ...
                        for _ in 0..steps.unsigned_abs() {
                            apply_command_and_journal(
                                ...
                                &self.notif_queue,
                                &mut self.quit_requested,
                                tessera_protocol::Command::Cycle { forward: steps > 0 },
                                &self.ipc,
                                &self.journal,
                                ts,
                                tessera_protocol::Origin::Gesture,
    ...
    fn dispatch_keybinding(&mut self, invocation: KeybindingInvocation, session_locked: bool) {
        ...
        let ts = self.start.elapsed().as_millis() as u64;
        let origin = tessera_protocol::Origin::Keybinding;
        match action {
            ...
            Action::CloseFocused => {
                if let Some(id) = self.server.focused_toplevel_id() {
                    let cmd = tessera_protocol::Command::Close { id };
                    apply_command_and_journal(
                        self.server,
                        &self.notif_queue,
                        &mut self.quit_requested,
            ...
            Action::CycleFocus => {
                ...
                let cmd = tessera_protocol::Command::Cycle { forward: true };
                apply_command_and_journal(
                    self.server,
                    &self.notif_queue,
                    &mut self.quit_requested,
            ...
            Action::CycleFocusBack => {
                ...
                let cmd = tessera_protocol::Command::Cycle { forward: false };
                apply_command_and_journal(
                    self.server,
                    &self.notif_queue,
                    &mut self.quit_requested,
            ...
            Action::WorkspaceNext => {
                let cmd = tessera_protocol::Command::SwitchWorkspace {
                    dir: tessera_desktop::workspace::Switch::Next,
                };
                apply_command_and_journal(
                    self.server,
                    &self.notif_queue,
            ...
            Action::WorkspacePrev => {
                let cmd = tessera_protocol::Command::SwitchWorkspace {
                    dir: tessera_desktop::workspace::Switch::Prev,
                };
                apply_command_and_journal(
                    self.server,
                    &self.notif_queue,
            ...
            Action::ToggleFullscreen => {
                ...
                if let Some(id) = self.server.focused_toplevel_id() {
                    let cmd = tessera_protocol::Command::SetFullscreen {
                        id,
            ...
            Action::Quit => {
                let cmd = tessera_protocol::Command::Quit;
                apply_command_and_journal(
                    self.server,
                    &self.notif_queue,
                    &mut self.quit_requested,
    ...
    pub(super) fn process_input(
        ...
        for (interaction_domain, damage) in self.server.take_interaction_domain_damage() {
            ...
            if let Some(ipc) = &self.ipc {
                ipc.broadcast(tessera_protocol::Event::InteractionDomainDamaged {
                    interaction_domain,
        ...
        let mut input = tessera_shell::component::Input::default();
                                    ...
                                    .filter(|action| {
                                        matches!(
                                            ...
                                            tessera_desktop::keybind::Action::CycleFocus
                                                | tessera_desktop::keybind::Action::CycleFocusBack
                                        )
                                    });
        ...
        for (cmd, ts, origin) in pending_synthetic_input {
            let effect = match &cmd {
                tessera_protocol::Command::InjectInput { id, actions } => {
                    ...
                    if let Some(events) = prepared {
                        ...
                        if blocked_by_chrome {
                            ...
                            let no_bindings = tessera_desktop::keybind::Keymap::default();
        ...
        if session_locked {
            ...
            input = tessera_shell::component::Input::default();
            input.set_display_size(self.input_acc.display_size.0, self.input_acc.display_size.1);
            input.set_cursor(-1.0, -1.0);
        } else {
...
mod tests {
    ...
    fn frame_state_merge_drops_the_fast_path_once_a_later_iteration_qualifies_out() {
        let mut first = frame(
            tessera_shell::component::Input::new((100.0, 80.0), 0.0),
            true,
        );
    ...
    fn frame_state_merge_preserves_edges_until_redraw() {
        let mut first = tessera_shell::component::Input::new((100.0, 80.0), 0.0);
        first
        ...
        let mut second = tessera_shell::component::Input::new((120.0, 90.0), 0.0);
        second
    ...
    fn applied_agent_actions_become_ordered_privacy_preserving_feedback() {
        ...
        assert_eq!(
            feedback[1].kind,
            tessera_shell::component::AgentInputKind::Click { button: 0x110 }
        );
