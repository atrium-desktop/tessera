...
pub(super) enum AppPickControl {
    ...
    Start {
        ...
        reply: std::sync::mpsc::Sender<Result<tessera_protocol::AppPickResult, String>>,
    },
...
pub(super) struct PendingAppPick {
    pub(super) conn_id: u64,
    pub(super) reply: std::sync::mpsc::Sender<Result<tessera_protocol::AppPickResult, String>>,
}
            ...
            match request.action {
                AppPickControl::Start {
                    ...
                    if self.pending_app_pick.is_some() || self.pending_pick.is_some() {
                        ...
                        self.shell
                            .start_app_pick(tessera_shell::component::AppPickParams {
                                choices,
                                subject,
                                last_choice,
                            });
