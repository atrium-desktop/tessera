...
) -> tessera_desktop::settings::DesktopPreferences {
...
pub(super) fn preferences_for_persistence(
    config: Option<&tessera_config::Config>,
    mut requested: tessera_desktop::settings::DesktopPreferences,
    overrides: &PreferenceOverrides,
) -> tessera_desktop::settings::DesktopPreferences {
...
pub(super) fn configured_output_modes(
    config: Option<&tessera_config::Config>,
) -> std::collections::HashMap<String, tessera_desktop::output::ModeSpec> {
    config
...
pub(super) fn configured_color_policies(
    config: Option<&tessera_config::Config>,
) -> std::collections::HashMap<String, tessera_desktop::output::ColorPolicy> {
    config
        .map(|c| c.output_policies())
        .unwrap_or_default()
        .into_iter()
        .map(|(connector, policy)| {
            (
                connector,
                tessera_desktop::output::ColorPolicy {
                    hdr: policy.hdr,
...
pub(super) fn apply_display_settings(
    settings: tessera_desktop::settings::DisplaySettings,
    config_path: Option<&std::path::Path>,
    ...
    system_status: &mut tessera_shell::component::SystemStatus,
...
pub(super) fn apply_desktop_preferences(
    preferences: tessera_desktop::settings::DesktopPreferences,
    config_path: Option<&std::path::Path>,
    config_writer: &ConfigWriter,
    config: &mut Option<tessera_config::Config>,
    keymap: &mut tessera_desktop::keybind::Keymap,
...
pub(super) fn output_geometry_from_host(
    ...
    logical_h: i32,
    scale: f32,
) -> tessera_desktop::output::OutputGeometry {
...
pub(super) fn build_keymap(
    config: Option<&tessera_config::Config>,
) -> tessera_desktop::keybind::Keymap {
    let mut overrides: Vec<tessera_desktop::keybind::Keybind> = Vec::new();
    ...
    tessera_desktop::keybind::Keymap::defaults()
...
const fn unavailable_key_action_feature(
    ...
    match action {
        tessera_desktop::keybind::Action::TogglePivot if !cfg!(feature = "chrome-pivot") => {
            Some("chrome-pivot")
        }
        tessera_desktop::keybind::Action::ToggleControlCenter
            if !cfg!(any(
                feature = "chrome-control-center",
...
pub(super) fn build_gesture_map(
    config: Option<&tessera_config::Config>,
) -> tessera_desktop::gesture::GestureMap {
    let mut overrides: Vec<tessera_desktop::gesture::GestureBinding> = Vec::new();
    ...
    tessera_desktop::gesture::GestureMap::defaults()
        .with_overrides(overrides)
        .retain_actions(|action| unavailable_gesture_action_feature(action).is_none())
}

const fn unavailable_gesture_action_feature(
    action: tessera_desktop::gesture::GestureAction,
) -> Option<&'static str> {
    match action {
        tessera_desktop::gesture::GestureAction::ControlCenter
            if !cfg!(any(
                feature = "chrome-control-center",
                feature = "chrome-command-panel"
            )) =>
...
/// registry.
pub(super) fn builtin_ipc_scopes() -> std::collections::HashMap<String, tessera_protocol::Scope> {
    std::collections::HashMap::from([
        (
            tessera_protocol::LOCAL_OWNER_ADMIN_SCOPE.to_string(),
            tessera_protocol::Scope {
                ...
                ops: Some(vec![
                    tessera_protocol::ActorCapability::Focus,
                    tessera_protocol::ActorCapability::Minimize,
                    tessera_protocol::ActorCapability::Close,
                    tessera_protocol::ActorCapability::SetWindowGeometry,
                    tessera_protocol::ActorCapability::SwitchWorkspace,
                    tessera_protocol::ActorCapability::SwitchWorkspaceTo,
                    tessera_protocol::ActorCapability::MoveToWorkspace,
                    tessera_protocol::ActorCapability::SystemControl,
                    tessera_protocol::ActorCapability::Notify,
                    tessera_protocol::ActorCapability::DismissNotification,
                    tessera_protocol::ActorCapability::Screenshot,
                    tessera_protocol::ActorCapability::ScreenshotRegion,
                    tessera_protocol::ActorCapability::ToggleOverview,
                ]),
                ask_ops: None,
            },
        ),
        (
            tessera_protocol::LOCAL_AGENT_ADMIN_SCOPE.to_string(),
            tessera_protocol::Scope {
        ...
        (
            tessera_protocol::LOCAL_INTERACTION_DOMAIN_ADMIN_SCOPE.to_string(),
            tessera_protocol::Scope {
                windows: None,
                workspaces: None,
                outputs: None,
                ...
                ops: Some(vec![
                    ...
                    tessera_protocol::ActorCapability::ObserveInteractionDomain,
        ...
        (
            tessera_protocol::LOCAL_PORTAL_SCOPE.to_string(),
            tessera_protocol::Scope {
                windows: None,
                workspaces: None,
                outputs: None,
                interaction_domains: None,
                ops: Some(vec![
                    tessera_protocol::ActorCapability::CaptureOutput,
                    tessera_protocol::ActorCapability::StreamOutput,
                    tessera_protocol::ActorCapability::IdleInhibit,
                    tessera_protocol::ActorCapability::PickTarget,
                    tessera_protocol::ActorCapability::PickApp,
                    tessera_protocol::ActorCapability::Notify,
                    tessera_protocol::ActorCapability::DismissNotification,
                    tessera_protocol::ActorCapability::PickConfirm,
                    tessera_protocol::ActorCapability::SetWallpaper,
                ]),
                ask_ops: None,
            },
        ),
...
pub(super) fn builtin_scope_executables(name: &str) -> Option<Vec<std::path::PathBuf>> {
    ...
    let paths: Vec<std::path::PathBuf> = match name {
        tessera_protocol::LOCAL_PORTAL_SCOPE => {
            ["/usr/bin", "/usr/libexec", "/usr/lib", "/usr/local/bin"]
                .into_iter()
                .map(|dir| executable(dir, "xdg-desktop-portal-atrium"))
                .collect()
        }
        tessera_protocol::LOCAL_OWNER_ADMIN_SCOPE
        | tessera_protocol::LOCAL_AGENT_ADMIN_SCOPE
        | tessera_protocol::LOCAL_INTERACTION_DOMAIN_ADMIN_SCOPE => ["/usr/bin", "/usr/local/bin"]
            .into_iter()
...
fn authorize_interaction_domain_action_groups_against_snapshot(
    ...
    let tessera_protocol::InteractionDomainAction::Transact { mutations, .. } = action else {
        return Ok(());
    };
    for mutation in mutations {
        let group = match mutation {
            ...
            tessera_authority::interaction_domain::InteractionDomainMutation::SetObserver {
                group,
                ..
            } => snapshot
                .interaction_groups
            ...
            tessera_authority::interaction_domain::InteractionDomainMutation::ConfigureOutput {
                ..
            }
            | tessera_authority::interaction_domain::InteractionDomainMutation::SetState {
                ..
            } => None,
        };
