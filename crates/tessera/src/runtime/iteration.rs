...
pub(super) struct PendingScreenshot {
    pub(super) command: tessera_protocol::Command,
...
pub(super) struct IterationWork {
    pub(super) pending_synthetic_input:
        Vec<(tessera_protocol::Command, u64, tessera_protocol::Origin)>,
...
impl CompositorRuntime<'_> {
    ...
    pub(super) fn prepare_iteration(
                ...
                snapshot
                    ...
                    .filter(|interaction_domain| {
                        ...
                        && interaction_domain.state
                            == tessera_authority::interaction_domain::InteractionDomainState::Active
                    })
            {
                if self
                    .server
                        ...
                        match encoded {
                            Ok(png) => {
                                ...
                                if let Err(error) = self.server.set_clipboard_data_shared(
                                    tessera_authority::interaction_domain::HUMAN_SEAT,
            ...
            )
        {
            ...
            self.system_status.display = tessera_desktop::settings::DisplayStatus {
            ...
            match request.action {
                StreamControl::Start {
                    ...
                    let result = if self.server.session_locked() || !self.host.is_active() {
                        ...
                        match &target {
                            tessera_protocol::StreamTarget::Output { output } => {
                            ...
                            tessera_protocol::StreamTarget::Window { window } => {
        ...
        while let Ok(request) = self.interaction_domain_control_rx.try_recv() {
            ...
            let allowed_while_locked = match &request.action {
                tessera_protocol::InteractionDomainAction::Revoke { .. } => true,
                tessera_protocol::InteractionDomainAction::Transact { mutations, .. } => {
                    !mutations.is_empty()
                ...
                tessera_protocol::InteractionDomainAction::Create { .. } => false,
            };
            let result = if self.server.session_locked() && !allowed_while_locked {
                Err("session is locked".into())
            } else {
                apply_interaction_domain_action(self.server, request.subject, request.action)
            };
            if result.is_ok() {
        ...
        while let Ok(request) = self.system_control_rx.try_recv() {
            ...
            let allowed_while_locked = matches!(
                &action,
                tessera_protocol::SystemAction::SetOutputPower { .. }
            );
            let result = if self.server.session_locked() && !allowed_while_locked {
                Err("session is locked".into())
            } else {
        ...
        while let Ok(request) = self.ipc_cmd_rx.try_recv() {
            ...
            let allowed_while_locked = matches!(
                &cmd,
                tessera_protocol::Command::System {
                    action: tessera_protocol::SystemAction::SetOutputPower { .. }
                }
            );
            ...
            if let tessera_protocol::Command::System { action } = &cmd {
            ...
            if let tessera_protocol::Command::LaunchInInteractionDomain {
                interaction_domain,
                desktop_id,
            } = &cmd
            {
                    ...
                    .find(|entry| entry.id == *desktop_id)
                {
                    ...
                    None => tessera_protocol::Effect::Refused {
            ...
            if let tessera_protocol::Command::LaunchApp {
                desktop_id,
                placement,
            } = &cmd
            {
                    ...
                    .find(|entry| entry.id == *desktop_id)
                {
                    Some(entry) => {
                        ...
                        match tessera_apps::launch(entry, &opts) {
                            ...
                            Err(error) => tessera_protocol::Effect::Refused {
            ...
            if matches!(cmd, tessera_protocol::Command::InjectInput { .. }) {
            ...
            if matches!(cmd, tessera_protocol::Command::ToggleOverview) {
            ...
            if matches!(cmd, tessera_protocol::Command::Screenshot { .. }) {
    ...
    pub(super) fn evaluate_night_light(&mut self) {
        use tessera_desktop::night_light as nl;
...
pub(super) fn screenshot_updates_human_clipboard(origin: &tessera_protocol::Origin) -> bool {
    matches!(
        origin,
        tessera_protocol::Origin::Chrome | tessera_protocol::Origin::Keybinding
    )
}

/// Local wall-clock time for the night-light schedule.
fn local_clock_now() -> tessera_desktop::night_light::ClockTime {
    unsafe {
        let now = libc::time(std::ptr::null_mut());
        let mut tm: libc::tm = std::mem::zeroed();
        libc::localtime_r(&now, &mut tm);
        tessera_desktop::night_light::ClockTime {
