...
pub(in crate::runtime) enum CaptureCompletion {
    ...
    InteractionDomainReply {
        reply:
            std::sync::mpsc::Sender<Result<tessera_ipc::CaptureInteractionDomainPayload, String>>,
        security_generation: u64,
        observation_token: tessera_protocol::ObservationToken,
    ...
    Pixel {
        reply: std::sync::mpsc::Sender<Result<tessera_protocol::PickResult, String>>,
        security_generation: u64,
        picked: Result<tessera_protocol::PickResult, String>,
    },
                    ...
                    match job {
                        CaptureJob::Screenshot {
                            ...
                            let written = encoded.as_ref().map_err(Clone::clone).and_then(|png| {
                                if permitted() {
                                    super::atomic_write_capture(&path, png)
                                } else {
                                    Err("session locked before capture completed".into())
                                }
                            });
...
mod tests {
    ...
    fn encoded_phase_keeps_the_bounded_lane_reserved() {
        let encoded = CaptureCompletion::ScreenshotEncoded {
            ...
            origin: tessera_protocol::Origin::Keybinding,
