...
//! lives in `tessera-render::capture`. This module owns only the orchestrating worker and its
...
pub use tessera_render::capture::{
    CaptureCursor, PendingReadback, StreamPixels, read_captured_pixels, read_captured_pixels_owned,
    request_frame_readback,
};
// Test-only capture helpers (BGRA conversion and PNG encoding assertions).
#[cfg(test)]
pub(crate) use tessera_render::capture::{encode_rgba_capture, stream_pixels};
...
pub use tessera_render::presentation::{
    clamp_logical_region, flux_last_error_detail, logical_rect_to_physical,
};
