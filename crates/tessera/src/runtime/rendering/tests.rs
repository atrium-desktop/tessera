...
fn region(x: f32, y: f32, w: f32, h: f32) -> tessera_shell::component::BackdropRegion {
    tessera_shell::component::BackdropRegion {
        x,
        y,
        w,
        h,
...
fn backdrop_cache_key_tracks_geometry_and_material_exactly() {
    ...
    let glass = tessera_shell::component::LiquidGlassRegion {
        ...
        focus: Some(tessera_shell::component::LiquidGlassFocus {
    ...
    let moved_focus = BackdropMaterialKey::new(
        &frost,
        &[tessera_shell::component::LiquidGlassRegion {
            focus: glass
                .focus
                .map(|focus| tessera_shell::component::LiquidGlassFocus {
                    bounds: region(
                        focus.bounds.x + 1.0,
                        focus.bounds.y,
                        focus.bounds.w,
    ...
    let adapted = BackdropMaterialKey::new(
        &frost,
        &[tessera_shell::component::LiquidGlassRegion {
            adaptation: Some(tessera_shell::component::LiquidGlassAdaptation {
                plate_luminance: 0.5,
...
fn focused_preview_content_keeps_one_full_brightness_target() {
    let focused = tessera_desktop::window::WindowId(7);
    let sibling = tessera_desktop::window::WindowId(8);
    assert_eq!(
        tessera_shell::component::preview::content_brightness(Some(focused), focused, 0.74),
        1.0
    );
    assert_eq!(
        tessera_shell::component::preview::content_brightness(Some(focused), sibling, 0.74),
        0.74
    );
    assert_eq!(
        tessera_shell::component::preview::content_brightness(None, sibling, 0.74),
        1.0
    );
}
...
fn backdrop_graph_accumulates_sampling_radius_across_three_layers() {
    use tessera_shell::component::{BackdropLayer, BackdropLayerId, BackdropLayerSource};
...
fn liquid_glass_geometry_maps_to_the_downsampled_capture_once() {
    let source = tessera_shell::component::LiquidGlassRegion {
...
fn opaque_frame_fill_replaces_undefined_and_previous_contents() {
    ...
    let mut canvas = flux::Canvas::new(&surface).unwrap();
...
fn damaged_base_and_stencil_overlay_preserve_pixels_outside_the_scissor() {
    ...
    let mut frame = surface.begin_frame().unwrap();
    let session =
        begin_opaque_frame(&mut canvas, &mut frame, flux::rgba(200, 30, 20, 255)).unwrap();
    session.end().unwrap();
    frame.submit().unwrap().present().unwrap();
    ...
    session.end().unwrap();
    let session =
        begin_stencil_frame_overlay(&mut canvas, &mut frame, frame_damage_render_area(&repaint))
            .unwrap();
    ...
    session.fill_path(
        &path,
        flux::FillRule::NonZero,
        &flux::Brush::solid(flux::rgba(20, 220, 70, 255)),
    );
    session.end().unwrap();
    frame.submit().unwrap().present().unwrap();
...
fn backdrop_frost_mapping_carries_opacity_and_wash_into_capture_pixels() {
    ...
    let washed = tessera_shell::component::BackdropRegion {
        x: 10.0,
        y: 20.0,
        w: 40.0,
        h: 30.0,
        wash: Some(tessera_shell::component::backdrop_wash(lens::Color::rgba(
            8, 10, 18, 126,
        ))),
        opacity: 0.25,
    };
...
fn frost_opacity_drains_the_cover_between_sharp_and_blurred() {
    ...
    let mut render = |opacity: f32| -> Vec<u8> {
        ...
        let session = canvas
            ...
            .unwrap();
        material.draw(&session, 0.0, 0.0, W as f32, H as f32);
        session.end().unwrap();
        frame.submit().unwrap().present().unwrap();
