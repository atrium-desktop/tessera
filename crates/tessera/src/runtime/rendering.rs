...
/// Begin a stencil-capable overlay pass over the currently presented content.
/// The pass loads the existing framebuffer contents; `render_area` from the
/// split preserves partial-repaint bounds while `clear: None` preserves every
/// pixel already composited below the UI.
pub(super) fn begin_stencil_frame_overlay<'c, 'surface>(
...
#[allow(dead_code)]
pub(super) fn begin_opaque_frame_repaint<'c, 'surface>(
    canvas: &'c mut flux::Canvas,
    frame: &'c mut flux::Frame<'surface>,
    _size: (u32, u32),
    clear: u32,
    ...
    let render_area = frame_damage_render_area(repaint);
    canvas.begin_session(
        frame,
        flux::CanvasPassOptions {
            clear: Some(clear),
...
#[allow(dead_code)]
pub(super) fn begin_opaque_target<'c, 'borrow, 'surface>(
    canvas: &'c mut flux::Canvas,
    frame: &'borrow mut flux::Frame<'surface>,
    target: &'borrow mut flux::Image,
    clear: u32,
) -> Result<flux::CanvasSession<'c, flux::ImageTarget<'borrow, 'surface>>, flux::Error> {
    debug_assert_eq!(clear >> 24, 0xff, "compositor pass clear must be opaque");
    canvas.begin_session(
        flux::ImageTarget {
            frame,
...
pub(super) fn blur_capture_regions(
    regions: &[tessera_shell::component::BackdropRegion],
...
pub(super) fn plan_backdrop_graph(
    ...
    for (layer, node) in layers.iter().zip(&nodes) {
        let source = match layer.source {
            tessera_shell::component::BackdropLayerSource::Scene => scene,
            tessera_shell::component::BackdropLayerSource::Layer(id) => {
                *ids.get(&id).ok_or_else(|| {
    ...
    let sources = order
        ...
        .map(|declaration| match layers[*declaration].source {
            tessera_shell::component::BackdropLayerSource::Scene => Ok(None),
            tessera_shell::component::BackdropLayerSource::Layer(id) => position_for_id
                .get(&id)
                .copied()
                .map(Some)
...
fn physical_backdrop_rect(
    region: tessera_shell::component::BackdropRegion,
    logical_size: (u32, u32),
    physical_size: (u32, u32),
    scale: f32,
...
pub(super) fn glass_region_active(region: &tessera_shell::component::LiquidGlassRegion) -> bool {
...
pub(super) fn liquid_glass_groups(
    regions: &[tessera_shell::component::LiquidGlassRegion],
    capture_origin: (u32, u32),
    scale: f32,
    capture_ratio: f32,
...
impl BackdropMaterialKey {
    pub(super) fn new(
        frost_regions: &[tessera_shell::component::BackdropRegion],
        liquid_regions: &[tessera_shell::component::LiquidGlassRegion],
        glass_tint: [u8; 3],
    ) -> Self {
        Self {
    ...
    pub(super) fn new(
        ...
        Self {
            ...
            layer_graph: layers
                ...
                .map(|layer| {
                    let source = match layer.source {
                        tessera_shell::component::BackdropLayerSource::Scene => u64::MAX,
                        tessera_shell::component::BackdropLayerSource::Layer(id) => id.0,
                    };
...
struct BackdropOperator {
    id: tessera_shell::component::BackdropLayerId,
...
impl BackdropGraphExecutor {
    ...
    pub(super) fn prepare(
        ...
        let positions: std::collections::HashMap<_, _> = layers
            ...
            .collect();
        for layer in layers {
            if let tessera_shell::component::BackdropLayerSource::Layer(source) = layer.source
    ...
    pub(super) fn begin_capture<'c, 'surface>(
        &'c mut self,
        canvas: &'c mut flux::Canvas,
        frame: &'c mut flux::Frame<'surface>,
        clear: u32,
        render_area: flux::CanvasRenderArea,
    ) -> Option<flux::CanvasSession<'c, flux::ImageTarget<'c, 'surface>>> {
        let slot = frame.index() as usize;
        let capture = self.captures.get_mut(slot)?.as_mut()?;
        let target = &mut capture.image;
        let result = canvas.begin_session(
            flux::ImageTarget {
                frame,
                image: target,
            },
    ...
    #[allow(dead_code)]
    pub(super) fn target(&self, slot: usize) -> Option<&flux::Image> {
        self.captures
            .get(slot)
    ...
    pub(super) fn target_mut(&mut self, slot: usize) -> Option<&mut flux::Image> {
        self.captures
            .get_mut(slot)?
            .as_mut()
            .map(|capture| &mut capture.image)
    }

    pub(super) fn capture_size(&self, slot: usize) -> Option<(u32, u32)> {
        self.captures
            .get(slot)
            .and_then(Option::as_ref)
            ...
            for (index, (operator, work)) in self.operators.iter_mut().zip(layers).enumerate() {
                ...
                if !material_regions.is_empty() {
                    ...
                    for region in &material_regions {
                        let session = canvas.begin_session(
                            flux::ImageTarget {
                                frame,
                                image: &mut current.composite,
                            },
                            flux::CanvasPassOptions {
                ...
                if let Some(resolved) = current.resolved.as_mut() {
...
impl ScreenshotFreeze {
    ...
    pub(super) fn mark_captured(&mut self, slot: usize) {
...
/// as a live thumbnail on the shared `tessera_shell::layout::overview` grid — the exact
...
pub(super) fn draw_overview_scene(
    ...
    let interaction_domain_shelf = server
        ...
        .any(|interaction_domain| {
            interaction_domain.kind
                == tessera_authority::interaction_domain::InteractionDomainKind::Agent
    ...
    let area = tessera_shell::layout::overview::grid_area_with_interaction_domain_shelf(
        display,
        rail,
...
/// its top-rail tile using the shared `tessera_shell::layout::overview` geometry, so
...
fn draw_workspace_rail_tiles(
    ...
    let tiles = tessera_shell::layout::overview::rail(display, output.workspaces.len());
    ...
    for (entry, tile) in output.workspaces.iter().zip(tiles.iter()) {
        let tile_windows: Vec<&tessera_desktop::window::Window> = entry
            .toplevels
            .iter()
        ...
        let content = tessera_shell::layout::overview::tile_content(*tile);
        let slots = tessera_shell::layout::overview::grid_with_spacing(
            content,
            tile_windows.len(),
            tessera_shell::layout::overview::TILE_GRID_MARGIN,
            tessera_shell::layout::overview::TILE_GRID_GAP,
        );
        let cells: std::collections::HashMap<
            tessera_desktop::window::WindowId,
            (
        ...
        > = tile_windows
            ...
            .map(|(window, slot)| {
                (
                    window.id,
                    (
                        tessera_shell::layout::overview::fit(*slot, window.size),
                        window.position,
                        window.size,
                    ),
                )
        ...
        let set: std::collections::HashSet<tessera_desktop::window::WindowId> =
...
pub(super) fn draw_window_switcher_cards(
    ...
    for card in &presentation.cards {
        ...
        let brightness = tessera_shell::component::preview::content_brightness(
...
fn draw_preview_card_scene(
    ...
    let cell = tessera_shell::layout::overview::fit(card.geometry.preview, window.size);
...
impl WindowShadowRenderer {
    ...
    pub(super) fn prepare(
        ...
        canvas: &mut flux::Canvas,
        frame: &mut flux::Frame<'_>,
        windows: &[tessera_desktop::window::Window],
        scale: f32,
        style: tessera_desktop::window::WindowShadowStyle,
    ) -> Vec<RenderedShadow> {
        if style != tessera_desktop::window::WindowShadowStyle::Soft {
            return Vec::new();
        }
        let slot = frame.index() as usize;
        let mut out = Vec::new();
            ...
            .filter(|w| tessera_render::window_casts_shadow(w))
        {
            ...
            let Some(mask) = self.masks[slot].as_mut() else {
                continue;
            };
            ...
            let draw_mask_ok = {
                let session = canvas.begin_session(
                    flux::ImageTarget { frame, image: mask },
                    flux::CanvasPassOptions {
                        clear: Some(clear),
