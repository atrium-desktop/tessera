...
pub(super) struct ObservationDiscardRequest {
    pub(super) actor: ActorBinding,
    pub(super) token: tessera_protocol::ObservationToken,
}
...
pub(super) struct TransactRequest {
    ...
    pub(super) ops: Vec<tessera_protocol::Command>,
    pub(super) reply: std::sync::mpsc::Sender<Result<tessera_protocol::TransactResult, String>>,
}
...
pub(super) struct SystemControlRequest {
    pub(super) origin: tessera_protocol::Origin,
    pub(super) action: tessera_protocol::SystemAction,
...
pub(super) struct InteractionDomainControlRequest {
    ...
    pub(super) action: tessera_protocol::InteractionDomainAction,
    pub(super) reply:
        std::sync::mpsc::Sender<Result<tessera_protocol::InteractionDomainActionResult, String>>,
}

pub(super) struct SettingsControlRequest {
    pub(super) origin: tessera_protocol::Origin,
    pub(super) expected_revision: Option<u64>,
    pub(super) action: tessera_protocol::SettingsAction,
    pub(super) reply: std::sync::mpsc::Sender<Result<tessera_protocol::SettingsReceipt, String>>,
}
        ...
        match action {
            ...
            tessera_protocol::InteractionDomainAction::Transact { mutations, .. } => {
                for mutation in mutations {
                    if let tessera_authority::interaction_domain::InteractionDomainMutation::SetState {
                        interaction_domain,
                        state,
                    } = mutation
                    {
...
pub(super) struct InteractionDomainRenderTarget {
    pub(super) output: tessera_authority::interaction_domain::VirtualOutput,
...
pub(super) fn virtual_output_physical_size(
    output: tessera_authority::interaction_domain::VirtualOutput,
...
pub(super) fn begin_interaction_domain_capture(
    ...
    let session = begin_opaque_frame(
        &mut target.canvas,
        &mut frame,
    ...
    renderer.draw_surfaces_ordered(device, &session, &surface_order, &shm, &dmabuf);
    session.restore();
    session.end().map_err(|error| {
        format!(
