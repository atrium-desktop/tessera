...
pub(super) struct LiveState {
    windows: std::sync::RwLock<Vec<tessera_desktop::window::Window>>,
    ...
    interaction_domains:
        std::sync::RwLock<tessera_authority::interaction_domain::InteractionDomainSnapshot>,
    settings: std::sync::RwLock<tessera_protocol::SettingsSnapshot>,
    system_status: std::sync::RwLock<tessera_protocol::SystemStatus>,
    notifications: std::sync::Arc<std::sync::Mutex<tessera_desktop::notify::NotificationQueue>>,
    journal:
        std::sync::Arc<std::sync::Mutex<tessera_audit::AuditLog<tessera_audit::journal::Entry>>>,
    ...
    scopes: std::sync::RwLock<std::collections::HashMap<String, tessera_protocol::Scope>>,
...
fn actor_action_scope_still_authorized(
    before: tessera_protocol::AuthorizationDecision,
    now: tessera_protocol::AuthorizationDecision,
    recorded: Option<bool>,
) -> bool {
    use tessera_protocol::AuthorizationDecision;
    match (before, now) {
        (
...
fn resource_confirmation(
    resource: &tessera_authority::authority::ActorResource,
) -> (String, String, Option<String>) {
    use tessera_authority::authority::{ActorResource, FilesystemAccess};
    match resource {
...
impl LiveState {
    ...
    pub(super) fn new(
        ...
        journal: std::sync::Arc<
            std::sync::Mutex<tessera_audit::AuditLog<tessera_audit::journal::Entry>>,
        >,
        scopes: std::collections::HashMap<String, tessera_protocol::Scope>,
        ...
        LiveState {
            ...
            settings: std::sync::RwLock::new(tessera_protocol::SettingsSnapshot::default()),
            system_status: std::sync::RwLock::new(tessera_protocol::SystemStatus::default()),
            notifications,
            journal,
            ...
            actor_sessions: std::sync::Mutex::new(
                tessera_authority::authority::ActorSessionRegistry::default(),
            ),
    ...
    fn actor_binding(
        ...
        (
            ActorBinding,
            tessera_authority::authority::ActorSessionSnapshot,
        ),
        String,
    > {
        let principal = subject
            .map(tessera_authority::authority::ActorPrincipal::new)
            .transpose()
    ...
    pub(super) fn revalidate_actor_action_scope(
        ...
        actor: &ActorBinding,
        authorized_scope: &tessera_protocol::Scope,
        interaction_domain: tessera_authority::interaction_domain::InteractionDomainId,
    ) -> Result<tessera_protocol::Scope, String> {
        ...
        let recorded = actor.principal.as_deref().and_then(|subject| {
            self.grants.read().unwrap().decision_for(
                subject,
                tessera_protocol::ActorCapability::InjectInteractionDomainInput,
            )
        });
    ...
    fn audit_event(
        &self,
        origin: tessera_protocol::Origin,
        mutation: tessera_protocol::JournalMutation,
    ) {
        self.persist_and_broadcast(origin, mutation, tessera_protocol::Effect::Applied);
    }
    ...
    fn persist_and_broadcast(
        ...
        origin: tessera_protocol::Origin,
        mutation: tessera_protocol::JournalMutation,
        effect: tessera_protocol::Effect,
    ) {
        ...
        self.journal_broadcaster.broadcast(entry.into());
    }
    ...
    fn auth_event(
        ...
        conn_id: Option<u64>,
        principal: &str,
        action: tessera_protocol::AgentAuthAction,
    ) {
        let origin = conn_id
            .map(|conn_id| tessera_protocol::Origin::ipc(conn_id, Some(principal)))
            .unwrap_or(tessera_protocol::Origin::Internal);
        self.audit_event(
            origin,
            tessera_protocol::JournalMutation::AgentAuth {
    ...
    fn resource_grant_event(
        &self,
        origin: tessera_protocol::Origin,
        grant: &tessera_authority::authority::ResourceGrant,
        action: tessera_protocol::ResourceGrantAuditAction,
    ) {
        self.audit_event(
            origin,
            tessera_protocol::JournalMutation::ResourceGrant {
    ...
    fn terminate_actor_session(
        &self,
        snapshot: tessera_authority::authority::ActorSessionSnapshot,
        action: tessera_protocol::ActorSessionAuditAction,
        origin: tessera_protocol::Origin,
    ) {
        ...
        self.audit_event(
            origin.clone(),
            tessera_protocol::JournalMutation::ActorSession {
                session: snapshot.id,
    ...
    pub(super) fn expire_due_actor_sessions(&self) {
        ...
        for snapshot in expired {
            self.terminate_actor_session(
                snapshot,
                tessera_protocol::ActorSessionAuditAction::Expired,
                tessera_protocol::Origin::Internal,
            );
        }
    }
    ...
    fn expire_due_resource_grants(&self) {
        ...
        for grant in expired {
            self.resource_grant_event(
                tessera_protocol::Origin::Internal,
                &grant,
                tessera_protocol::ResourceGrantAuditAction::Expired,
            );
        }
    }
    ...
    pub(super) fn set_workspaces(&self, snapshot: tessera_desktop::workspace::WorkspaceSnapshot) {
        *self.workspaces.write().unwrap() = snapshot;
    }

    pub(super) fn set_outputs(&self, outputs: Vec<tessera_desktop::output::OutputInfo>) {
        *self.outputs.write().unwrap() = outputs;
    }

    pub(super) fn set_interaction_domains(
        &self,
        snapshot: tessera_authority::interaction_domain::InteractionDomainSnapshot,
    ) {
        *self.interaction_domains.write().unwrap() = snapshot;
    }

    pub(super) fn set_settings(&self, snapshot: tessera_protocol::SettingsSnapshot) {
...
impl tessera_ipc::Handler for LiveState {
    ...
    fn policy_caps(&self) -> tessera_protocol::ConnectionCapabilities {
        tessera_protocol::ConnectionCapabilities {
            query: true,
            control: true,
            input: true,
            session: true,
    ...
    fn notifications(&self) -> Vec<tessera_desktop::notify::Notification> {
        self.notifications.lock().unwrap().snapshot()
    }

    fn outputs(&self) -> Vec<tessera_desktop::output::OutputInfo> {
        self.outputs.read().unwrap().clone()
    }

    fn journal_since(&self, since: u64) -> tessera_protocol::JournalSnapshot {
        self.journal.lock().unwrap().since(since).into()
    }

    fn interaction_domains(
        &self,
    ) -> tessera_authority::interaction_domain::InteractionDomainSnapshot {
        self.interaction_domains.read().unwrap().clone()
    }

    fn settings(&self) -> tessera_protocol::SettingsSnapshot {
        self.settings.read().unwrap().clone()
    }

    fn system_status(&self) -> tessera_protocol::SystemStatus {
        self.system_status.read().unwrap().clone()
    }

    fn authorize_interaction_domain_action(
        &self,
        scope: &tessera_protocol::Scope,
        action: &tessera_protocol::InteractionDomainAction,
    ) -> Result<(), String> {
    ...
    fn audit_refusal(
        ...
        self.persist_and_broadcast(
            tessera_protocol::Origin::ipc(conn_id, subject),
            mutation,
            tessera_protocol::Effect::Refused { reason },
        );
    }
    ...
    fn audit_capability_use(
        ...
        subject: Option<&str>,
        session: tessera_authority::authority::ActorSessionId,
        capability: tessera_protocol::ActorCapability,
        action: tessera_protocol::CapabilityUseAction,
        effect: tessera_protocol::Effect,
    ) {
        self.persist_and_broadcast(
            tessera_protocol::Origin::ipc(conn_id, subject),
            tessera_protocol::JournalMutation::CapabilityUse {
                session,
                principal: subject.and_then(|value| {
                    tessera_authority::authority::ActorPrincipal::new(value).ok()
                }),
                capability,
                action,
            },
    ...
    fn command(&self, conn_id: u64, subject: Option<&str>, cmd: tessera_protocol::Command) {
        ...
        let _ = self.commands.lock().unwrap().send(IpcCommandRequest {
            origin: tessera_protocol::Origin::ipc(conn_id, subject),
            command: cmd,
        });
    }
    ...
    fn transact(
        ...
        ops: Vec<tessera_protocol::Command>,
    ) -> Result<tessera_protocol::TransactResult, String> {
    ...
    fn system_action(
        ...
        conn_id: u64,
        subject: Option<&str>,
        action: tessera_protocol::SystemAction,
    ) -> Result<(), String> {
        let (reply_tx, reply_rx) = std::sync::mpsc::channel();
    ...
    fn resolve_scope(&self, name: &str) -> Option<tessera_protocol::Scope> {
    ...
    fn agent_lookup(
        &self,
        credential: &str,
    ) -> Option<tessera_authority::authority::AgentIdentity> {
    ...
    fn refresh_agent_identity(
        &self,
        principal: &str,
    ) -> Result<Option<tessera_authority::authority::AgentIdentity>, String> {
        self.agent_auth
            .read()
            .unwrap()
    ...
    fn start_actor_session(
        ...
        policy: tessera_authority::authority::ActorSessionPolicy,
    ) -> Result<tessera_authority::authority::ActorSessionSnapshot, String> {
        ...
        self.audit_event(
            ...
            tessera_protocol::JournalMutation::ActorSession {
                ...
                action: tessera_protocol::ActorSessionAuditAction::Started,
            },
        );
        Ok(snapshot)
    }
    ...
    fn issue_resource_grant(
    ...
    ) -> Result<tessera_authority::authority::ResourceGrant, String> {
        self.expire_due_resource_grants();
        ...
        if confirm_exact_resource {
            ...
            if self.pick_confirm(conn_id, title, body, accept_label)?
                != tessera_protocol::ConfirmPickResult::Confirmed
            {
        ...
        self.resource_grant_event(
            ...
            &grant,
            tessera_protocol::ResourceGrantAuditAction::Issued,
        );
        Ok(grant)
    }
    ...
    fn consume_resource_grant(
        ...
        session: tessera_authority::authority::ActorSessionId,
        principal: Option<&str>,
        id: &tessera_authority::authority::ResourceGrantId,
        resource: &tessera_authority::authority::ActorResource,
    ) -> Result<tessera_authority::authority::ResourceGrant, String> {
        self.expire_due_resource_grants();
        let session_snapshot = self.actor_sessions.lock().unwrap().authorize(session)?;
        ...
        self.resource_grant_event(
            ...
            &grant,
            tessera_protocol::ResourceGrantAuditAction::Consumed,
        );
        Ok(grant)
    }
    ...
    fn pair_agent(
        ...
        conn_id: u64,
        label: Option<&str>,
        requested: &[tessera_protocol::ActorCapability],
    ) -> Result<tessera_authority::authority::PairedAgent, String> {
        {
        ...
        let families = capability_families(requested)
            .into_iter()
            .map(|family| tessera_shell::component::CapabilityFamily {
                key: family.key.to_owned(),
                label: family.label.to_owned(),
                members: family
                    .members
                    .into_iter()
                    .map(|member| tessera_shell::component::CapabilityGroup {
        ...
        self.capability_pick_controls
            ...
            .send(CapabilityPickControlRequest {
                ...
                action: CapabilityPickControl::Start {
                    params: tessera_shell::component::CapabilityPickParams {
                        title,
                        warning,
                        families,
                    },
        ...
        match reply_rx.recv_timeout(PICK_TIMEOUT) {
            Ok(Ok(tessera_shell::component::CapabilityPickResult {
                approved: Some(keys),
            })) => {
                let ops: Vec<tessera_protocol::ActorCapability> = keys
                    .iter()
                    .filter_map(|key| tessera_protocol::ActorCapability::from_name(key))
                    .collect();
                let paired = self.agent_auth.write().unwrap().issue(label, &ops)?;
                self.auth_event(
                    Some(conn_id),
                    &paired.principal,
                    tessera_protocol::AgentAuthAction::Paired,
                );
                Ok(paired)
            }
            Ok(Ok(tessera_shell::component::CapabilityPickResult { approved: None })) => {
    ...
    fn grant_for(&self, principal: &str, op: tessera_protocol::ActorCapability) -> Option<bool> {
    ...
    fn request_grant(
        ...
        conn_id: u64,
        principal: &str,
        op: tessera_protocol::ActorCapability,
    ) -> Result<bool, String> {
        let label = self
            .agent_auth
            .read()
        ...
        self.confirm_pick_controls
            ...
            .send(ConfirmPickControlRequest {
                ...
                action: ConfirmPickControl::Start {
                    ...
                    body,
                    accept_label: None,
                    style: tessera_shell::component::ConfirmPickStyle::Grant,
                    reply: reply_tx,
                },
            })
            .map_err(|_| "compositor is shutting down".to_owned())?;
        ...
        // chrome so it never lingers for a dead requester.
        match reply_rx.recv_timeout(PICK_TIMEOUT) {
            Ok(Ok(tessera_shell::component::ConfirmAnswer::AllowOnce)) => {
                self.auth_event(
                    Some(conn_id),
                    principal,
                    tessera_protocol::AgentAuthAction::Granted {
                        op,
                        persistence: tessera_protocol::GrantPersistence::Once,
                    },
                );
                Ok(true)
            }
            Ok(Ok(tessera_shell::component::ConfirmAnswer::AllowSession)) => {
                self.grants
                    .write()
                    .unwrap()
                ...
                self.auth_event(
                    ...
                    tessera_protocol::AgentAuthAction::Granted {
                        op,
                        persistence: tessera_protocol::GrantPersistence::Session,
                    },
                );
                Ok(true)
            }
            Ok(Ok(tessera_shell::component::ConfirmAnswer::AllowAlways)) => {
                self.grants
                    .write()
                    .unwrap()
                ...
                self.auth_event(
                    ...
                    tessera_protocol::AgentAuthAction::Granted {
                        op,
                        persistence: tessera_protocol::GrantPersistence::Always,
                    },
                );
                Ok(true)
            }
            Ok(Ok(_)) => {
                ...
                self.auth_event(
                    ...
                    tessera_protocol::AgentAuthAction::Granted {
                        op,
                        persistence: tessera_protocol::GrantPersistence::DeniedSession,
                    },
                );
                Ok(false)
            }
    ...
    fn agent_principals(&self) -> Vec<tessera_protocol::AgentPrincipalInfo> {
        self.agent_auth
            .read()
            .unwrap()
            .principals()
            .iter()
            .map(|record| tessera_protocol::AgentPrincipalInfo {
    ...
    fn agent_grants(&self, principal: Option<&str>) -> Vec<tessera_protocol::AgentGrantInfo> {
    ...
    fn rename_agent_principal(&self, principal: &str, label: Option<&str>) -> Result<(), String> {
        ...
        self.auth_event(None, principal, tessera_protocol::AgentAuthAction::Renamed);
        Ok(())
    }
    ...
    fn forget_agent_principal(&self, principal: &str) -> Result<(), String> {
        ...
        let principal =
            tessera_authority::authority::ActorPrincipal::new(principal).map_err(str::to_owned)?;
        let revoked = self
            .actor_sessions
            .lock()
            .unwrap()
        ...
        for snapshot in revoked {
            self.terminate_actor_session(
                snapshot,
                tessera_protocol::ActorSessionAuditAction::PrincipalRevoked,
                tessera_protocol::Origin::Internal,
            );
        }
        ...
        self.auth_event(
            None,
            principal.as_ref(),
            tessera_protocol::AgentAuthAction::Forgotten,
        );
        Ok(())
    }
    ...
    fn set_agent_ceiling(
        &self,
        principal: &str,
        pregranted: &[tessera_protocol::ActorCapability],
        gated: &[tessera_protocol::ActorCapability],
    ) -> Result<(), String> {
        ...
        self.auth_event(
            None,
            principal,
            tessera_protocol::AgentAuthAction::CeilingChanged,
        );
        Ok(())
    }
    ...
    fn register_agent(
        ...
        self.auth_event(None, &principal, tessera_protocol::AgentAuthAction::Paired);
    ...
    fn revoke_agent_grant(
        ...
        self.auth_event(
            None,
            principal,
            tessera_protocol::AgentAuthAction::GrantRevoked { op },
        );
        Ok(())
    }
    ...
    fn interaction_domain_action(
        ...
        subject: Option<&str>,
        action: tessera_protocol::InteractionDomainAction,
    ) -> Result<tessera_protocol::InteractionDomainActionResult, String> {
        let (reply_tx, reply_rx) = std::sync::mpsc::channel();
    ...
    fn window_capture_target_exists(&self, window: tessera_desktop::window::WindowId) -> bool {
        self.all_windows
            .read()
            .unwrap()
            .iter()
    ...
    fn act_in_interaction_domain(
        ...
        scope_name: Option<&str>,
        scope: tessera_protocol::Scope,
        intent: tessera_protocol::ActorActionIntent,
    ) -> Result<tessera_protocol::ActorActionReceipt, String> {
        let (reply_tx, reply_rx) = std::sync::mpsc::channel();
    ...
    fn connection_disconnected(&self, conn_id: u64) {
        ...
        for snapshot in revoked {
            let origin = tessera_protocol::Origin::ipc(
                conn_id,
                snapshot
                    .principal
                    .as_ref()
            ...
            self.terminate_actor_session(
                snapshot,
                tessera_protocol::ActorSessionAuditAction::Disconnected,
                origin,
            );
        }
    }
    ...
    fn discard_observation(
        ...
        conn_id: u64,
        subject: Option<&str>,
        token: &tessera_protocol::ObservationToken,
    ) {
    ...
    fn pick_target(
        &self,
        conn_id: u64,
        kind: tessera_protocol::PickKind,
    ...
    fn pick_confirm(
    ...
    ) -> Result<tessera_protocol::ConfirmPickResult, String> {
        let (reply_tx, reply_rx) = std::sync::mpsc::channel();
        self.confirm_pick_controls
            .lock()
            .unwrap()
            .send(ConfirmPickControlRequest {
                ...
                action: ConfirmPickControl::Start {
                    ...
                    body,
                    accept_label,
                    style: tessera_shell::component::ConfirmPickStyle::YesNo,
                    reply: reply_tx,
                },
            })
            .map_err(|_| "compositor is shutting down".to_owned())?;
        ...
        match reply_rx.recv_timeout(PICK_TIMEOUT) {
            Ok(Ok(tessera_shell::component::ConfirmAnswer::Confirmed)) => {
                Ok(tessera_protocol::ConfirmPickResult::Confirmed)
            }
            Ok(Ok(_)) => Ok(tessera_protocol::ConfirmPickResult::Cancelled),
            Ok(Err(message)) => Err(message),
            Err(_) => {
                let _ =
                    self.confirm_pick_controls
    ...
    ) -> Result<tessera_protocol::SecretPromptResult, String> {
        let (reply_tx, reply_rx) = std::sync::mpsc::channel();
    ...
    ) -> Result<tessera_protocol::AppPickResult, String> {
        let (reply_tx, reply_rx) = std::sync::mpsc::channel();
...
mod actor_action_scope_tests {
    use super::*;
    use tessera_protocol::{ActorCapability, AuthorizationDecision};
