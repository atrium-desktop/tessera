...
/// Dispatch an [`tessera_protocol::Command`] to the server and side-effect targets. Extracted
...
pub(super) fn apply_command(
    ...
    notif_queue: &std::sync::Arc<std::sync::Mutex<tessera_desktop::notify::NotificationQueue>>,
    quit: &mut bool,
    cmd: &tessera_protocol::Command,
    ...
    match cmd {
        ...
        Command::Notify {
            ...
            if let Some(s) = ipc.as_ref() {
                s.broadcast(tessera_protocol::Event::Notified { notification: n });
            }
        }
...
pub(super) fn apply_interaction_domain_action(
    ...
    match action {
        tessera_protocol::InteractionDomainAction::Create {
            label,
            capabilities,
            output,
        } => {
            ...
            Ok(tessera_protocol::InteractionDomainActionResult::Created { bundle })
        }
        tessera_protocol::InteractionDomainAction::Transact {
            expected_revision,
            mutations,
        } => server
            .transact_interaction_domains(expected_revision, &mutations)
            .map(
                |receipt| tessera_protocol::InteractionDomainActionResult::TransactionCommitted {
                    receipt,
                },
            )
            .map_err(|error| error.to_string()),
        tessera_protocol::InteractionDomainAction::Revoke {
            interaction_domain,
            fallback,
            expected_revision,
        } => {
            ...
            server
                ...
                .map(|receipt| tessera_protocol::InteractionDomainActionResult::Revoked { receipt })
                .map_err(|error| error.to_string())
        }
    }
}
...
pub(super) fn interaction_domain_intent_to_action(
    intent: tessera_shell::component::InteractionDomainIntent,
) -> tessera_protocol::InteractionDomainAction {
    match intent {
        tessera_shell::component::InteractionDomainIntent::TransferWindow {
            window,
            target,
...
pub(super) fn interaction_domain_action_invalidates_capture(
    action: &tessera_protocol::InteractionDomainAction,
) -> std::collections::BTreeSet<tessera_authority::interaction_domain::InteractionDomainId> {
    match action {
        tessera_protocol::InteractionDomainAction::Create { .. } => {
        ...
        tessera_protocol::InteractionDomainAction::Transact { mutations, .. } => mutations
            .iter()
            .filter_map(|mutation| match mutation {
                tessera_authority::interaction_domain::InteractionDomainMutation::SetState {
                    interaction_domain,
                    state:
                        tessera_authority::interaction_domain::InteractionDomainState::Paused
    ...
    match action {
        ...
        tessera_protocol::InteractionDomainAction::Transact { mutations, .. } => mutations
            ...
            .filter_map(|mutation| match mutation {
                tessera_authority::interaction_domain::InteractionDomainMutation::SetState {
                    interaction_domain,
                    state: tessera_authority::interaction_domain::InteractionDomainState::Paused,
                } => Some(*interaction_domain),
                _ => None,
            })
            .collect(),
...
) -> tessera_protocol::JournalEntry {
    journal_mutation_effect_and_broadcast(
        journal,
        ipc,
        ts_mono_ms,
        origin,
        tessera_protocol::JournalMutation::Command {
            cmd: tessera_protocol::AuditedCommand::from(&cmd),
        },
        effect,
    )
}
...
pub(super) fn journal_mutation_effect_and_broadcast(
    ...
    let entry = match journal.try_append(ts_mono_ms, origin, mutation, effect) {
        Ok(entry) => tessera_protocol::JournalEntry::from(entry),
        Err(error) => {
...
pub(super) fn apply_command_and_journal(
    ...
    quit: &mut bool,
    command: tessera_protocol::Command,
    ipc: &Option<tessera_ipc::Server>,
    journal: &std::sync::Arc<
        std::sync::Mutex<tessera_audit::AuditLog<tessera_audit::journal::Entry>>,
    >,
    ...
    let effect = match apply_command(server, notifications, quit, &command, ipc, ts_mono_ms) {
        Ok(()) => tessera_protocol::Effect::Applied,
...
pub(super) fn apply_transact_batch(
    ...
    for cmd in ops {
        ...
        after_seq = seq;
        results.push(tessera_protocol::TransactOpResult { seq, effect });
    }
...
mod tests {
    ...
    impl Fixture {
        fn new() -> Self {
            Self {
                server: tessera_wayland::Server::new().expect("Server::new"),
                notifications: std::sync::Arc::new(std::sync::Mutex::new(
                    tessera_desktop::notify::NotificationQueue::new(60_000),
                )),
                journal: std::sync::Arc::new(std::sync::Mutex::new(tessera_audit::AuditLog::<
                    tessera_audit::journal::Entry,
                >::default_capacity(
                ))),
                quit: false,
            }
        ...
        fn transact(
            ...
            apply_transact_batch(
                ...
                &self.journal,
                1,
                tessera_protocol::Origin::Ipc { conn_id: 1 },
    ...
    fn transact_batch_commits_in_order_with_per_op_receipts() {
        ...
        let result = fixture
            .transact(
                ...
                None,
                vec![
                    tessera_protocol::Command::Notify {
                        summary: "s1".into(),
                        body: "b1".into(),
                        app_id: None,
                        external_id: None,
            ...
            .expect("batch");
        let tessera_protocol::TransactResult::Committed { receipt } = result else {
        ...
        assert_eq!(
            receipt
                ...
                .collect::<Vec<_>>(),
            vec![
                (1, &tessera_protocol::Effect::Applied),
                (2, &tessera_protocol::Effect::Applied)
            ]
        );
        assert_eq!(
    ...
    fn transact_batch_precondition_conflicts_apply_nothing() {
        ...
        let result = fixture
            .transact(
                None,
                None,
                vec![tessera_protocol::Command::Notify {
                    summary: "n1".into(),
                    body: "b1".into(),
                    app_id: None,
                    external_id: None,
        ...
        assert_eq!(
            result,
            tessera_protocol::TransactResult::PreconditionConflict {
                precondition: tessera_protocol::TransactPrecondition::JournalSeq,
                expected: 0,
                actual: 1,
            }
        ...
        assert!(
            matches!(result, tessera_protocol::TransactResult::Committed { .. }),
        ...
        let result = fixture
            .transact(
                ...
                vec![
                    ...
                    tessera_protocol::Command::Focus {
                        id: tessera_desktop::window::WindowId(99),
                        reveal: true,
                    },
                    tessera_protocol::Command::Notify {
                        summary: "s2".into(),
