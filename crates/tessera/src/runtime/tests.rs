...
fn only_user_initiated_screenshots_update_the_human_clipboard() {
    ...
    assert!(screenshot_updates_human_clipboard(
        &tessera_protocol::Origin::Keybinding
    ));
    assert!(!screenshot_updates_human_clipboard(
        &tessera_protocol::Origin::Ipc { conn_id: 7 }
    ));
    assert!(!screenshot_updates_human_clipboard(
...
fn builtin_scopes_are_fail_closed_allowlists() {
    ...
    let owner = scopes
        .get(tessera_protocol::LOCAL_OWNER_ADMIN_SCOPE)
        .expect("built-in owner scope");
    assert!(owner.permits(&tessera_protocol::Command::Focus {
        id: tessera_desktop::window::WindowId(9),
        reveal: true,
    }));
    assert!(
        !owner.permits(&tessera_protocol::Command::LaunchInInteractionDomain {
            interaction_domain: tessera_authority::interaction_domain::InteractionDomainId(9),
            desktop_id: "foot.desktop".into(),
        })
    );
    assert!(!owner.permits(&tessera_protocol::Command::LaunchApp {
        desktop_id: "foot.desktop".into(),
        placement: None,
    }));
    assert!(!owner.permits(&tessera_protocol::Command::LaunchApp {
        desktop_id: "foot.desktop".into(),
        placement: Some(tessera_desktop::workspace::LaunchPlacement::Workspace {
            id: tessera_desktop::workspace::WorkspaceId(9),
        }),
    }));

    let agent_admin = scopes
        .get(tessera_protocol::LOCAL_AGENT_ADMIN_SCOPE)
        .expect("built-in agent administration scope");
    assert_eq!(agent_admin.ops.as_deref(), Some([].as_slice()));

    let admin = scopes
        .get(tessera_protocol::LOCAL_INTERACTION_DOMAIN_ADMIN_SCOPE)
        .expect("built-in InteractionDomain recovery scope");
    assert!(
        admin.permits(&tessera_protocol::Command::LaunchInInteractionDomain {
            interaction_domain: tessera_authority::interaction_domain::InteractionDomainId(9),
            desktop_id: "foot.desktop".into(),
        })
    );
    assert!(!admin.permits(&tessera_protocol::Command::LaunchApp {
        desktop_id: "foot.desktop".into(),
        placement: None,
    }));
    assert!(!admin.permits(&tessera_protocol::Command::LaunchApp {
    ...
    let portal = scopes
        .get(tessera_protocol::LOCAL_PORTAL_SCOPE)
        .expect("built-in portal scope");
    assert!(!portal.permits(&tessera_protocol::Command::System {
        action: tessera_protocol::SystemAction::ToggleMute,
    }));
}
...
fn interaction_domain_scope_expands_atomic_groups_before_authorizing() {
    ...
    let first = tessera_desktop::window::WindowId(7);
    let sibling = tessera_desktop::window::WindowId(8);
    let group = model
        ...
        .unwrap();
    let action = tessera_protocol::InteractionDomainAction::Transact {
    ...
    let one_window = tessera_protocol::Scope {
    ...
    let complete_group = tessera_protocol::Scope {
    ...
    let observe = tessera_protocol::InteractionDomainAction::Transact {
        expected_revision: None,
        mutations: vec![
...
fn automation_operation_names_accept_canonical_and_snake_case() {
    assert_eq!(
        tessera_protocol::ActorCapability::from_name("SetWindowGeometry"),
        Some(tessera_protocol::ActorCapability::SetWindowGeometry)
    );
    assert_eq!(
        tessera_protocol::ActorCapability::from_name("set_window_geometry"),
        Some(tessera_protocol::ActorCapability::SetWindowGeometry)
    );
    assert_eq!(
        tessera_protocol::ActorCapability::from_name("inject_input"),
        Some(tessera_protocol::ActorCapability::InjectInput)
    );
    assert_eq!(
        tessera_protocol::ActorCapability::from_name("system_control"),
        Some(tessera_protocol::ActorCapability::SystemControl)
    );
}

#[test]
fn builtin_scope_executables_cover_every_builtin_scope() {
    let portal = builtin_scope_executables(tessera_protocol::LOCAL_PORTAL_SCOPE).unwrap();
    assert_eq!(
        portal,
        vec![
