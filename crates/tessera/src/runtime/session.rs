...
pub(super) struct IdleProcess {
    ...
    settings: tessera_desktop::settings::IdleSettings,
    nested: bool,
    available: bool,
    stopping: bool,
...
impl IdleProcess {
    pub(super) fn start(
        ...
        let mut process = Self {
            ...
            mode: tessera_desktop::power::PowerMode::default(),
        };
        if available {
            process.spawn();
        } else {
    ...
    pub(super) fn reconfigure(&mut self, settings: tessera_desktop::settings::IdleSettings) {
    ...
    pub(super) fn set_mode(&mut self, mode: tessera_desktop::power::PowerMode) {
        if self.mode == mode {
            return;
        }
        self.mode = mode;
    ...
    pub(super) fn mode(&self) -> tessera_desktop::power::PowerMode {
        self.mode
    }
