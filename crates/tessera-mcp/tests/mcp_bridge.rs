...
use tessera_protocol::{
    ActorCapability, ConnectionCapabilities, Effect, InteractionDomainAction,
    InteractionDomainActionResult, JournalMutation, Origin,
};
type Journal = tessera_audit::AuditLog<tessera_audit::journal::Entry>;
...
struct TestHandler {
    ...
    notifications: Mutex<Vec<tessera_desktop::notify::Notification>>,
...
impl Handler for TestHandler {
    fn pair_agent(
        ...
        Ok(tessera_authority::authority::PairedAgent {
            principal: tessera_protocol::ActorPrincipal::new("prin_test").unwrap(),
    ...
    fn workspaces(&self) -> tessera_desktop::workspace::WorkspaceSnapshot {
        tessera_desktop::workspace::WorkspaceSnapshot { outputs: vec![] }
    }
    ...
    fn command(&self, conn_id: u64, subject: Option<&str>, cmd: tessera_protocol::Command) {
        if let tessera_protocol::Command::Notify {
            summary,
            body,
            app_id,
            external_id: _,
        } = &cmd
        {
            ...
            notifications.push(tessera_desktop::notify::Notification {
                id,
    ...
    assert!(
        handler
            ...
            .any(|entry| matches!(
                &entry.mutation,
                JournalMutation::Command {
                    cmd: tessera_protocol::AuditedCommand::LaunchApp { desktop_id, workspace }
