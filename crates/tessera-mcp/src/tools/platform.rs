...
impl TesseraPlatform {
    ...
    pub fn connect(config: BridgeConfig) -> Result<Self, PlatformError> {
        config.validate()?;
        let identity =
            tessera_ipc::IdentityStore::load(config.data_dir.clone(), &config.instance_id);
        let mut conn = tessera_ipc::PersistentConnection::connect(tessera_ipc::ConnectParams {
            ...
            credential: tessera_ipc::CredentialSource::Paired(identity),
    ...
    fn wait_for_notification(
        ...
        client: &mut Client,
        summary: &str,
    ) -> Result<tessera_desktop::notify::Notification, PlatformError> {
    ...
    fn command(
        ...
        // contract (ADR-0125).
        if let Some(op) = tessera_protocol::TransactOp::from_command(&command) {
...
fn run<T>(
    conn: &mut tessera_ipc::PersistentConnection,
    timeout: Duration,
    ...
    match conn.run(f) {
        Ok(value) => Ok(value),
        Err(tessera_ipc::PersistentError::Connect(error)) => Err(connect_error(error)),
        Err(tessera_ipc::PersistentError::Call(error)) => Err(error),
    }
}

fn connect_error(error: tessera_ipc::ConnectError) -> PlatformError {
    match error {
        tessera_ipc::ConnectError::Ipc {
            socket,
            label,
            source,
        ...
        tessera_ipc::ConnectError::MissingIdentity => PlatformError::MissingAuthenticatedIdentity,
        tessera_ipc::ConnectError::Identity(error) => PlatformError::Identity(error.to_string()),
    }
}
