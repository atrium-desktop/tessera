...
pub struct TesseraPlatform {
    ...
    conn: tessera_ipc::PersistentConnection,
}
...
pub enum PlatformError {
    ...
    InteractionDomain(#[from] tessera_ipc::SessionError),
}

impl tessera_ipc::ClassifyError for PlatformError {
    fn is_transport_failure(&self) -> bool {
        match self {
            PlatformError::Ipc(error) => tessera_ipc::is_transport_failure(error),
            PlatformError::Connect { source, .. } => tessera_ipc::is_transport_failure(source),
            PlatformError::InteractionDomain(tessera_ipc::SessionError::Io(error)) => {
                tessera_ipc::is_transport_failure(error)
            }
            _ => false,
        }
    }
