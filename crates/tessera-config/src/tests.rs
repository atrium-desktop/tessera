...
fn dock_position_defaults_to_bottom_and_parses_kebab_case_edges() {
    ...
    assert_eq!(
        cfg.dock.position,
        tessera_desktop::dock::DockPosition::Bottom
    );

    for (spelling, want) in [
        ("left", tessera_desktop::dock::DockPosition::Left),
        ("bottom", tessera_desktop::dock::DockPosition::Bottom),
        ("right", tessera_desktop::dock::DockPosition::Right),
    ] {
...
fn config_store_writes_the_dock_minimize_animation() {
    ...
    ConfigStore::new(&path)
        .apply(ConfigEdit::SetDockMinimizeAnimation {
            style: tessera_desktop::dock::MinimizeAnimationStyle::Scale,
        })
        .unwrap();
    let cfg = load(&path).unwrap().expect("file still valid");
    assert_eq!(
        cfg.dock.minimize_animation,
        tessera_desktop::dock::MinimizeAnimationStyle::Scale
    );
...
fn dock_minimize_animation_defaults_to_genie_and_parses_every_style() {
    use tessera_desktop::dock::MinimizeAnimationStyle;
    let cfg = Config::parse("schema_version = 2\n").unwrap();
...
fn config_store_writes_the_dock_minimize_animation_without_touching_pins() {
    ...
    assert_eq!(
        cfg.dock.minimize_animation,
        tessera_desktop::dock::MinimizeAnimationStyle::Suck
    );
    assert_eq!(cfg.dock.pinned, vec!["a.desktop"], "pins survive the edit");
    assert_eq!(
        cfg.dock.position,
        tessera_desktop::dock::DockPosition::Left,
    ...
    let err = ConfigStore::new(&path)
        .apply(ConfigEdit::SetOutput {
            settings: tessera_desktop::settings::DisplaySettings {
                ...
                mode: tessera_desktop::output::ModeSpec {
                    width: 1920,
                    height: 1080,
                    refresh_hz: Some(60),
                },
...
fn gesture_entry_resolves_to_gesture_binding() {
    ...
    assert_eq!(
        binds[0].axis,
        tessera_desktop::gesture::GestureAxis::Horizontal
    );
    assert_eq!(
        binds[0].action,
        tessera_desktop::gesture::GestureAction::WorkspaceSwitch
    );
}
...
fn gesture_map_layers_overrides_on_defaults() {
    ...
    // Override present.
    assert_eq!(
        gm.lookup(3, tessera_desktop::gesture::GestureAxis::Vertical),
        Some(tessera_desktop::gesture::GestureAction::CommandPanel)
    );
    // Defaults still present.
    assert_eq!(
        gm.lookup(3, tessera_desktop::gesture::GestureAxis::Horizontal),
        Some(tessera_desktop::gesture::GestureAction::WorkspaceSwitch)
    );
    assert!(gm.len() >= 4);
}
...
fn layout_section_overrides_defaults() {
    ...
    let p = tessera_desktop::layout::LayoutParams::from(cfg.layout);
    assert_eq!(p.gaps, 16);
}
...
fn ui_window_decorations_default_to_borderless_and_accept_client_side() {
    ...
    assert_eq!(
        ...
        tessera_desktop::window::DecorationPolicy::Borderless
    );
    ...
    assert_eq!(
        ...
        tessera_desktop::window::DecorationPolicy::ClientSide
    );
...
fn window_rules_parse_from_toml() {
    ...
    assert_eq!(
        ...
        Some(tessera_desktop::layout::LayoutRole::Tiled)
    );
    assert_eq!(cfg.window_rules[1].title.as_deref(), Some("calculator"));
    assert_eq!(
        cfg.window_rules[1].role,
        Some(tessera_desktop::layout::LayoutRole::Floating)
    );
