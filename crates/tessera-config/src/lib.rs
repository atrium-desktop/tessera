...
//!
//! The crate is pure: it depends on [`tessera_desktop`] for the shared keymap model
...
use tessera_desktop::keybind::Keybind;
use tessera_desktop::keybind::Keymap;
pub use tessera_desktop::settings::AccentColor;
pub use tessera_desktop::settings::BatterySettings;
pub use tessera_desktop::settings::ColorScheme;
...
pub struct Config {
    ...
    /// layout role). See [`tessera_desktop::window_rule::WindowRule`].
    #[serde(default, rename = "window_rule")]
    pub window_rules: Vec<tessera_desktop::window_rule::WindowRule>,
...
pub struct DockConfig {
    ...
    #[serde(default)]
    pub position: tessera_desktop::dock::DockPosition,
    #[serde(default)]
    pub minimize_animation: tessera_desktop::dock::MinimizeAnimationStyle,
}
...
impl Default for DockConfig {
    fn default() -> Self {
        Self {
            ...
            position: tessera_desktop::dock::DockPosition::default(),
...
pub struct UiConfig {
    ...
    #[serde(default)]
    pub window_decorations: tessera_desktop::window::DecorationPolicy,
    ...
    #[serde(default)]
    pub window_shadow: tessera_desktop::window::WindowShadowStyle,
}
...
/// built-in `tessera_desktop::layout::LayoutParams`.
#[derive(Debug, Clone, PartialEq, serde::Deserialize)]
#[serde(deny_unknown_fields)]
...
impl From<LayoutConfig> for tessera_desktop::layout::LayoutParams {
    fn from(c: LayoutConfig) -> tessera_desktop::layout::LayoutParams {
        tessera_desktop::layout::LayoutParams {
            gaps: c.gaps,
...
///
/// Field names match the [`tessera_desktop::keybind`] name resolvers: `mods` is a
...
/// triggers.
///
/// Field names match the [`tessera_desktop::gesture`] name resolvers. Unknown
...
impl Config {
    ...
    pub fn parse(text: &str) -> Result<Config, Vec<Diagnostic>> {
        ...
        for (index, output) in cfg.outputs.iter().enumerate() {
            ...
            if let Some(mode) = &output.mode {
                match mode.parse::<tessera_desktop::output::ModeSpec>() {
                    Ok(spec) => {
    ...
    pub fn resolve_keybinds(&self) -> (Vec<Keybind>, Vec<Diagnostic>) {
        ...
        for (i, entry) in self.keybinds.iter().enumerate() {
            ...
            for m in &entry.mods {
                match tessera_desktop::keybind::mod_from_name(m) {
                    Some(bit) => mods |= bit,
                    None => {
            ...
            let Some(keysym) = tessera_desktop::keybind::keysym_from_name(&entry.key) else {
                errs.push(Diagnostic::new(
                    Some(field.clone()),
            ...
            let Some(action) = tessera_desktop::keybind::action_from_name(&entry.action) else {
                errs.push(Diagnostic::new(
                    Some(field.clone()),
    ...
    /// [`tessera_desktop::gesture::GestureBinding`]s.
    ...
    pub fn resolve_gestures(
        ...
        Vec<tessera_desktop::gesture::GestureBinding>,
        Vec<Diagnostic>,
    ) {
        let mut binds = Vec::new();
        let mut errs = Vec::new();
        for (i, entry) in self.gestures.iter().enumerate() {
            ...
            let Some(axis) = tessera_desktop::gesture::gesture_axis_from_name(&entry.axis) else {
                errs.push(Diagnostic::new(
                    Some(field.clone()),
            ...
            let Some(action) = tessera_desktop::gesture::gesture_action_from_name(&entry.action)
            else {
                errs.push(Diagnostic::new(
                    Some(field.clone()),
                    format!("unknown action '{}'", entry.action),
            ...
            binds.push(tessera_desktop::gesture::GestureBinding {
    ...
    /// Build the active [`tessera_desktop::gesture::GestureMap`]: built-in defaults
    ...
    pub fn gesture_map(&self) -> (tessera_desktop::gesture::GestureMap, Vec<Diagnostic>) {
        let (overrides, errs) = self.resolve_gestures();
        (
            tessera_desktop::gesture::GestureMap::defaults().with_overrides(overrides),
            errs,
        )
    }

    /// Resolve the `[[output]]` entries into per-connector
    /// [`tessera_desktop::output::OutputPolicy`]s (ADR-0028). Mode and transform
    ...
    pub fn output_policies(
        &self,
    ) -> std::collections::HashMap<String, tessera_desktop::output::OutputPolicy> {
    ...
    match (&config.start, &config.end) {
        (Some(start), Some(end)) => {
            for (field, value) in [("start", start), ("end", end)] {
                if tessera_desktop::night_light::ClockTime::from_hhmm(value).is_none() {
                    diagnostics.push(Diagnostic::new(
...
pub enum ConfigEdit {
    ...
    SetDockMinimizeAnimation {
        style: tessera_desktop::dock::MinimizeAnimationStyle,
    },
...
fn apply_output(document: &mut DocumentMut, settings: tessera_desktop::settings::DisplaySettings) {
    let tessera_desktop::settings::DisplaySettings {
        connector,
        mode,
        scale,
        position,
...
fn format_mode_spec(mode: tessera_desktop::output::ModeSpec) -> String {
