...
pub const MAX_BLOB_BYTES: u64 = 288 * 1024 * 1024;
...
impl SealedBlob {
    ...
    pub fn send(&self, stream: &UnixStream) -> io::Result<()> {
        send_fd(stream, self.file.as_raw_fd())
    }
}

pub fn receive(stream: &UnixStream, expected_len: u64) -> io::Result<Vec<u8>> {
...
pub fn receive_fd(stream: &UnixStream) -> io::Result<RawFd> {
    let mut marker = 0u8;
    let mut iov = libc::iovec {
        iov_base: (&mut marker as *mut u8).cast(),
        iov_len: 1,
