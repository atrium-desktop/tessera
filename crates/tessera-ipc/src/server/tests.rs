    ...
    server.streams.lock().unwrap().insert(
        ...
        StreamLane {
            ...
            scope: LiveScopeBinding {
                connection_id: 1,
                session: tessera_authority::authority::ActorSessionId(1),
                name: None,
                principal: None,
                fallback: Scope::unscoped(),
            },
            target: tessera_protocol::schema::StreamTarget::Output { output: None },
            version,
...
fn slow_event_and_journal_subscribers_are_evicted_without_blocking() {
    ...
    server
        .journal_broadcaster
        .broadcast(tessera_protocol::journal::JournalEntry {
            seq: 1,
            ts_mono_ms: 1,
            origin: tessera_protocol::journal::Origin::Internal,
            mutation: tessera_protocol::journal::JournalMutation::CapabilityUse {
                session: tessera_authority::authority::ActorSessionId(1),
                principal: None,
                capability: ActorCapability::IdleInhibit,
                action: tessera_protocol::journal::CapabilityUseAction::Enable,
            },
            effect: tessera_protocol::journal::Effect::Applied,
        });
    assert!(
        server
            .journal_broadcaster
...
fn authenticated_subject_cannot_cross_agent_interaction_domain_ownership() {
    ...
    let own_revoke = InteractionDomainAction::Revoke {
        ...
        fallback: tessera_authority::interaction_domain::HUMAN_INTERACTION_DOMAIN,
...
struct StreamHandler {
    ...
    targets: Mutex<Vec<tessera_protocol::schema::StreamTarget>>,
    cursors: Mutex<Vec<tessera_protocol::schema::StreamCursorMode>>,
}
...
impl Handler for StreamHandler {
    ...
    fn enumerate_outputs(&self) -> Vec<tessera_protocol::schema::OutputInfo> {
        vec![
    ...
    fn journal_since(&self, _since: u64) -> tessera_protocol::journal::JournalSnapshot {
    ...
    fn stream_output_start(
        ...
        allow_dmabuf: bool,
        cursor: tessera_protocol::schema::StreamCursorMode,
...
fn stream_hello(version: u32) -> Request {
    Request::Hello {
        ...
        lease: Some(tessera_protocol::schema::LeaseRequest { ttl_ms: 60_000 }),
        agent: None,
    }
}
...
fn slot_frames_cross_the_wire_without_a_blob() {
    ...
    write_stream_frame(
        ...
        &handler,
        &scope,
        tessera_protocol::schema::StreamTarget::Output { output: None },
        &lease_deadline,
        &streams,
    )
    .unwrap();
...
fn stream_start_forwards_the_output_selector_and_cursor_mode() {
    ...
    std::thread::scope(|scope| {
        ...
        write_msg(
            ...
            &Request::StreamOutputStart {
                max_fps: None,
                target: tessera_protocol::schema::StreamTarget::Output {
                    output: Some("HDMI-A-1".into()),
                },
                dmabuf: None,
                cursor: Some(tessera_protocol::schema::StreamCursorMode::Embedded),
            },
        )
        .unwrap();
        match recv_response(&rx) {
    ...
    assert_eq!(
        ...
        &[
            tessera_protocol::schema::StreamTarget::Output {
                output: Some("HDMI-A-1".into()),
            },
            tessera_protocol::schema::StreamTarget::Output { output: None },
        ]
    ...
    assert_eq!(
        ...
        &[
            tessera_protocol::schema::StreamCursorMode::Embedded,
            tessera_protocol::schema::StreamCursorMode::Hidden,
        ]
    );
}
...
fn window_target_may_opt_into_dmabuf_at_version_29() {
    ...
    std::thread::scope(|scope| {
        ...
        write_msg(
            ...
            &Request::StreamOutputStart {
                max_fps: None,
                target: tessera_protocol::schema::StreamTarget::Window {
    ...
    assert_eq!(
        handler.targets.lock().unwrap().as_slice(),
        &[tessera_protocol::schema::StreamTarget::Window {
            window: tessera_desktop::window::WindowId(7),
        }]
    );
}
...
struct ScopeHandler {
    ...
    allowed: Vec<PathBuf>,
    refusals: Mutex<Vec<(u64, tessera_protocol::journal::JournalMutation, String)>>,
    lockdown: bool,
}
...
impl Handler for ScopeHandler {
    ...
    fn audit_refusal(
        ...
        conn_id: u64,
        _subject: Option<&str>,
        mutation: tessera_protocol::journal::JournalMutation,
        reason: String,
    ) {
        self.refusals
            .lock()
...
fn builtin_scope_claim_is_refused_for_a_foreign_executable() {
    ...
    match &refusals[0] {
        (7, tessera_protocol::journal::JournalMutation::ScopeClaim { scope }, reason) => {
