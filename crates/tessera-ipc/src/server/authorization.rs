...
pub(super) fn scope_permits_stream(
    scope: &Scope,
    target: &tessera_protocol::schema::StreamTarget,
) -> bool {
    scope.pregrants(ActorCapability::StreamOutput)
        && match target {
            ...
            tessera_protocol::schema::StreamTarget::Output { .. } => true,
            tessera_protocol::schema::StreamTarget::Window { window } => {
                scope.permits_window(*window)
            }
        }
}
...
pub(super) fn valid_agent_hello(agent: &tessera_protocol::schema::AgentHello) -> bool {
    agent
        .label
        .as_deref()
...
pub(super) fn valid_principal_id(principal: &str) -> bool {
    tessera_authority::authority::ActorPrincipal::new(principal.to_owned()).is_ok()
}
...
pub(super) fn valid_wallpaper_path(path: &Path) -> bool {
    tessera_authority::authority::ActorResource::FilesystemPath {
        ...
        access: tessera_authority::authority::FilesystemAccess::Read,
    }
    .validate()
    .is_ok()
}
...
pub(super) fn filter_windows(
    ...
    scope: &Scope,
) -> Vec<tessera_desktop::window::Window> {
    windows
        .into_iter()
...
pub(super) fn filter_interaction_domains(
    mut snapshot: tessera_authority::interaction_domain::InteractionDomainSnapshot,
    scope: &Scope,
    subject: Option<&str>,
    ...
    snapshot.interaction_domains.retain(|interaction_domain| {
        ...
        interaction_domain.kind
            == tessera_authority::interaction_domain::InteractionDomainKind::Human
            || principal_subjects
...
pub(super) fn filter_journal(
    mut snapshot: tessera_protocol::journal::JournalSnapshot,
    scope: &Scope,
    subject: Option<&str>,
) -> tessera_protocol::journal::JournalSnapshot {
    snapshot
        .entries
...
pub(super) fn journal_entry_permitted(
    entry: &tessera_protocol::journal::JournalEntry,
    scope: &Scope,
    subject: Option<&str>,
) -> bool {
    if let Some(subject) = subject
        && let tessera_protocol::journal::Origin::Actor { principal, .. } = &entry.origin
        && principal != subject
    {
        return false;
    }
...
pub(super) fn audit_resource_grant_refusal<H: Handler>(
    ...
    action: tessera_protocol::journal::ResourceGrantAttemptAction,
    resource: Option<&tessera_authority::authority::ActorResource>,
) {
    let reason = match action {
        tessera_protocol::journal::ResourceGrantAttemptAction::Issue => {
            "resource grant issue refused"
        }
        tessera_protocol::journal::ResourceGrantAttemptAction::Consume => {
            "resource grant consume refused"
        }
        tessera_protocol::journal::ResourceGrantAttemptAction::Revoke => {
            "resource grant revoke refused"
        }
    };
    handler.audit_refusal(
        ...
        JournalMutation::ResourceGrantAttempt {
            session,
            principal: subject
                .and_then(|value| tessera_authority::authority::ActorPrincipal::new(value).ok()),
            action,
            capability: resource
                .map(tessera_authority::authority::ActorResource::required_capability),
            resource_kind: resource.map(tessera_protocol::journal::ResourceKind::from),
        },
        reason.to_owned(),
    );
}
...
pub(super) fn audit_capability_response<H: Handler>(
    ...
    action: tessera_protocol::journal::CapabilityUseAction,
    response: &Response,
) {
    let effect = if matches!(response, Response::Error { .. }) {
        tessera_protocol::journal::Effect::Refused {
            reason: "capability use refused".into(),
        }
    } else {
        tessera_protocol::journal::Effect::Applied
    };
...
pub(super) fn audit_capability_effect<H: Handler>(
    ...
    capability: ActorCapability,
    action: tessera_protocol::journal::CapabilityUseAction,
    effect: tessera_protocol::journal::Effect,
) {
...
pub(super) fn subject_may_transfer_through(
    ...
    interaction_domain == tessera_authority::interaction_domain::HUMAN_INTERACTION_DOMAIN
...
pub(super) fn authorize_subject_interaction_domain_action(
    ...
    let allowed = match action {
        ...
        InteractionDomainAction::Transact { mutations, .. } => mutations.iter().all(|mutation| {
            match mutation {
