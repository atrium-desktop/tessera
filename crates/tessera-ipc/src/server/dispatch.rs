...
) -> Option<Vec<tessera_protocol::schema::OutputInfo>> {
...
pub(super) fn drive_read_loop<H: Handler>(
    ...
    let session_policy = tessera_authority::authority::ActorSessionPolicy {
        ttl: session_ttl,
        idle_timeout: session_ttl.min(std::time::Duration::from_secs(5 * 60)),
        ..tessera_authority::authority::ActorSessionPolicy::default()
    };
        ...
        let resp = match req {
            ...
            Request::Observe => {
                ...
                if !granted.query {
                    ...
                    let journal_cursor = observe(ActorCapability::ObserveJournal).then(|| {
                        ...
                        tessera_protocol::schema::JournalCursor {
                            oldest_seq: snapshot.oldest_seq,
                            latest_seq: snapshot.latest_seq,
                        }
                    });
                    Response::Observed {
                        snapshot: tessera_protocol::schema::ObserveSnapshot {
                            windows,
                            workspaces,
                            outputs,
                            notifications,
                ...
                if let Some(message) = rejection {
                    handler.audit_refusal(
                        ...
                        JournalMutation::Command {
                            cmd: tessera_protocol::journal::AuditedCommand::from(&cmd),
                        },
                        message.clone(),
                    );
                    Response::Error { message }
                            ...
                            for cmd in &commands {
                                ...
                                if let Some(message) = op_rejection {
                                    handler.audit_refusal(
                                        ...
                                        principal.as_deref(),
                                        JournalMutation::Command {
                                            cmd: tessera_protocol::journal::AuditedCommand::from(
                                                cmd,
                                            ),
                                        },
                                        message.clone(),
            ...
            Request::CaptureOutput { region } => {
                ...
                let op_allowed = current_scope.as_ref().is_some_and(|scope| {
                    scope.ops.as_ref().is_some_and(|ops| {
                        ops.contains(&tessera_protocol::schema::ActorCapability::CaptureOutput)
                    })
                });
                let response = if !granted.control {
                    Response::Error {
                ...
                audit_capability_response(
                    ...
                    tessera_protocol::journal::CapabilityUseAction::Capture,
                    &response,
                );
                response
            }
            ...
            Request::StreamOutputStart {
                ...
                audit_capability_response(
                    ...
                    tessera_protocol::journal::CapabilityUseAction::Start,
                    &response,
                );
                response
            }
            ...
            Request::StreamOutputStop { stream_id } => {
                ...
                audit_capability_response(
                    ...
                    &live_scope,
                    ActorCapability::StreamOutput,
                    tessera_protocol::journal::CapabilityUseAction::Stop,
                    &response,
                );
                response
            }
            Request::SetIdleInhibit { inhibit } => {
                ...
                audit_capability_response(
                    ...
                    if inhibit {
                        tessera_protocol::journal::CapabilityUseAction::Enable
                    } else {
                        tessera_protocol::journal::CapabilityUseAction::Disable
                    },
                    &response,
                );
                response
            ...
            Request::PickTarget { kind } => {
                ...
                audit_capability_response(
                    ...
                    tessera_protocol::journal::CapabilityUseAction::Pick,
                    &response,
                );
                response
            }
            ...
            Request::PromptSecret {
                ...
                let response = if !granted.control {
                    ...
                    let resource = tessera_authority::authority::ActorResource::secret_prompt(
                        &title,
                        reason.as_deref(),
                    );
                    match handler.consume_resource_grant(
                ...
                audit_capability_response(
                    ...
                    tessera_protocol::journal::CapabilityUseAction::Prompt,
                    &response,
                );
                response
            }
            ...
            Request::SetWallpaper { path } => {
                ...
                audit_capability_response(
                    ...
                    tessera_protocol::journal::CapabilityUseAction::Apply,
                    &response,
                );
                response
            }
