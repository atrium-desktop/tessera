...
use tessera_protocol::journal::JournalSnapshot;
use tessera_protocol::schema::{
...
pub struct CapturedInteractionDomain {
    pub interaction_domain: tessera_authority::interaction_domain::InteractionDomainId,
    pub width: u32,
    pub height: u32,
    pub scale_milli: u32,
...
pub struct Client {
    ...
    session: Option<tessera_protocol::ActorSessionSnapshot>,
...
impl Client {
    ...
    pub fn session(&self) -> Option<&tessera_protocol::ActorSessionSnapshot> {
        self.session.as_ref()
    }
    ...
    pub fn windows(&mut self) -> io::Result<Vec<tessera_desktop::window::Window>> {
    ...
    pub fn workspaces(&mut self) -> io::Result<tessera_desktop::workspace::WorkspaceSnapshot> {
    ...
    pub fn switch_workspace(&mut self, dir: tessera_desktop::workspace::Switch) -> io::Result<()> {
    ...
    pub fn notifications(&mut self) -> io::Result<Vec<tessera_desktop::notify::Notification>> {
    ...
    pub fn outputs(&mut self) -> io::Result<Vec<tessera_desktop::output::OutputInfo>> {
        write_msg(&mut self.stream, &Request::GetOutputs)?;
        match read_msg::<_, Response>(&mut self.stream)? {
            Response::Outputs { outputs } => Ok(outputs
                .into_iter()
                .map(|output| tessera_desktop::output::OutputInfo {
    ...
    pub fn interaction_domains(
        &mut self,
    ) -> io::Result<tessera_authority::interaction_domain::InteractionDomainSnapshot> {
    ...
    pub fn connection_state(
        ...
        Scope,
        Option<LeaseGrant>,
        Option<tessera_protocol::ActorSessionSnapshot>,
    )> {
