...
fn authenticated_capture_rechecks_the_live_principal_ceiling() {
    let path = scratch();
    let handler = Arc::new(TestHandler::permissive(sample_windows()));
    let identity = tessera_authority::authority::AgentIdentity {
        principal: tessera_authority::authority::ActorPrincipal::new("prin_capture").unwrap(),
...
fn wrong_protocol_version_is_refused_at_handshake() {
    ...
    let resp: tessera_protocol::Response = tessera_ipc::codec::read_msg(&mut s).unwrap();
    match resp {
        tessera_protocol::Response::Error { message } => {
            assert!(
...
fn ipc_origins_are_unique_and_pre_dispatch_refusals_are_audited() {
    ...
    assert!(matches!(
        ...
        tessera_protocol::JournalMutation::Command {
            cmd: tessera_protocol::AuditedCommand::Close { id: WindowId(1) }
        }
    ));
}
...
fn resource_grants_revalidate_scope_and_audit_refusals_without_secrets() {
    ...
    let resource = tessera_protocol::ActorResource::FilesystemPath {
        path: PathBuf::from("/private/customer.db"),
        access: tessera_protocol::FilesystemAccess::Read,
    };
    let mut client =
    ...
    assert!(matches!(
        mutation,
        tessera_protocol::JournalMutation::ResourceGrantAttempt {
            action: tessera_protocol::ResourceGrantAttemptAction::Consume,
            capability: Some(ActorCapability::ReadFile),
            resource_kind: Some(tessera_protocol::ResourceKind::FilesystemPath),
            ..
        }
    ));
...
fn switch_workspace_command_is_queued() {
    ...
    .expect("connect");
    client
        .switch_workspace(tessera_desktop::workspace::Switch::Next)
        .expect("switch");
    let recorded = handler.commands.lock().unwrap();
    assert!(
        recorded
...
fn notified_event_carries_the_notification() {
    ...
    let mut client = Client::connect(&path).expect("connect");
    client.subscribe().expect("subscribe");
    let n = tessera_desktop::notify::Notification {
        id: 7,
        summary: "ping".into(),
        body: "pong".into(),
        app_id: None,
