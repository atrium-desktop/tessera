...
fn accessibility_handler() -> (Arc<TestHandler>, PathBuf) {
    ...
    *handler.pair_result.lock().unwrap() = Ok(tessera_authority::authority::PairedAgent {
        principal: tessera_authority::authority::ActorPrincipal::new("prin_a11y").unwrap(),
...
fn dispatched_request() -> tessera_semantic::SemanticActionRequest {
    tessera_semantic::SemanticActionRequest {
        request_id: 7,
        target: tessera_semantic::model::SemanticObjectId::for_window(WindowId(1)),
        provider_node_id: 3,
        tree_revision: 11,
...
fn delivered_action_and_handler_refusal_are_audited() {
    ...
    *handler.next_action_result.lock().unwrap() = Err("pending-action quota exhausted".into());
    ...
    assert_eq!(uses[0].2, tessera_protocol::CapabilityUseAction::Await);
    assert_eq!(uses[0].3, tessera_protocol::Effect::Applied);
    assert_eq!(
        uses[1].3,
