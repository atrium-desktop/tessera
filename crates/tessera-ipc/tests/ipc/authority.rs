...
fn stream_output_start_requires_control_and_an_explicit_scope_op() {
    ...
    assert_eq!(started.format, tessera_protocol::StreamPixelFormat::Bgra8);
...
fn pick_target_requires_control_and_an_explicit_scope_op() {
    ...
    let result = scoped
        .pick_target(tessera_protocol::PickKind::Region)
    ...
    assert_eq!(
        ...
        &[(1, tessera_protocol::PickKind::Region)]
    );
    ...
    let mut focus_scope =
        Client::connect_scoped(&path, requested, "focus-first").expect("scoped connect");
    let err = focus_scope
        .pick_target(tessera_protocol::PickKind::Pixel)
        .unwrap_err();
    assert!(err.to_string().contains("out of scope"), "{err}");
    ...
    let mut unscoped = Client::connect_with(&path, requested).expect("unscoped connect");
    let err = unscoped
        .pick_target(tessera_protocol::PickKind::Window)
        .unwrap_err();
    assert!(err.to_string().contains("out of scope"), "{err}");
...
fn stream_output_start_forwards_a_window_target() {
    ...
    scoped
        .start_output_stream_target(
            Some(30),
            tessera_protocol::StreamTarget::Window {
                window: WindowId(2),
            },
        )
    ...
    assert!(matches!(
        ...
        InteractionDomainActionResult::Created {
            bundle: tessera_authority::interaction_domain::InteractionDomainBundle {
                interaction_domain: tessera_authority::interaction_domain::InteractionDomainId(2),
                ..
            },
            ..
        }
