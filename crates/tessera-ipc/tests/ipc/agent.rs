...
fn pairing_issues_credential_and_synthetic_scope() {
    ...
    *handler.pair_result.lock().unwrap() = Ok(tessera_authority::authority::PairedAgent {
        principal: tessera_authority::authority::ActorPrincipal::new("prin_1").unwrap(),
...
fn authenticated_actor_observation_is_an_explicit_capability() {
    ...
    *handler.pair_result.lock().unwrap() = Ok(tessera_authority::authority::PairedAgent {
        principal: tessera_authority::authority::ActorPrincipal::new("prin_observer").unwrap(),
    ...
    *handler.pair_result.lock().unwrap() = Ok(tessera_authority::authority::PairedAgent {
        principal: tessera_authority::authority::ActorPrincipal::new("prin_observer_2").unwrap(),
...
fn recognized_credential_binds_without_pairing() {
    let path = scratch();
    let handler = Arc::new(TestHandler::permissive(vec![]));
    *handler.lookup_result.lock().unwrap() = Some(tessera_authority::authority::AgentIdentity {
        principal: tessera_authority::authority::ActorPrincipal::new("prin_9").unwrap(),
...
fn builtin_scope_connections_do_not_pair() {
    ...
    let client = Client::connect_agent_with_timeout(
        &path,
        ConnectionCapabilities::QUERY,
        Some(tessera_protocol::LOCAL_INTERACTION_DOMAIN_ADMIN_SCOPE.to_string()),
        tessera_protocol::AgentHello {
...
fn declared_scope_pairs_but_keeps_the_configured_ceiling() {
    ...
    let handler = Arc::new(TestHandler::permissive(vec![]));
    *handler.pair_result.lock().unwrap() = Ok(tessera_authority::authority::PairedAgent {
        principal: tessera_authority::authority::ActorPrincipal::new("prin_2").unwrap(),
...
fn grant_paired_handler(
    ...
    let handler = Arc::new(TestHandler::permissive(vec![]));
    *handler.pair_result.lock().unwrap() = Ok(tessera_authority::authority::PairedAgent {
        principal: tessera_authority::authority::ActorPrincipal::new("prin_g").unwrap(),
    ...
    *handler.request_grant_result.lock().unwrap() = Ok(true);
    ...
    let result = client
        .interaction_domain_action(InteractionDomainAction::Create {
            label: "agent".into(),
            capabilities: tessera_authority::interaction_domain::SeatCapabilities::POINTER_KEYBOARD,
            output: None,
        })
...
fn agent_management_round_trips() {
    ...
    handler
        ...
        .lock()
        .unwrap()
        .push(tessera_protocol::AgentPrincipalInfo {
    ...
    handler.grant_infos.lock().unwrap().extend([
        tessera_protocol::AgentGrantInfo {
            principal: "prin_1".into(),
        ...
        tessera_protocol::AgentGrantInfo {
            ...
            decision: tessera_protocol::AgentGrantDecision::Deny,
            granted_at: 3,
        },
    ]);
...
fn lockdown_exempts_builtin_scope_connections() {
    ...
    let portal = Client::connect_scoped(&path, privileged, tessera_protocol::LOCAL_PORTAL_SCOPE)
