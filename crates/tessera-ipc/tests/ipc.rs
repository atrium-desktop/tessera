...
use tessera_protocol::{
    ActorCapability, Command, ConnectionCapabilities, Event, InteractionDomainAction,
    InteractionDomainActionResult, PROTOCOL_VERSION, Scope, SettingsAction, SettingsReceipt,
    SettingsSnapshot, SystemAction, SystemStatus,
};
...
struct TestHandler {
    ...
    refusals: Mutex<Vec<(u64, tessera_protocol::JournalMutation, String)>>,
    ...
    stream_targets: Mutex<Vec<tessera_protocol::StreamTarget>>,
    stream_cursors: Mutex<Vec<tessera_protocol::StreamCursorMode>>,
    enumerated: Vec<tessera_protocol::OutputInfo>,
    picks: Mutex<Vec<(u64, tessera_protocol::PickKind)>>,
    ...
    capability_uses: Mutex<
        Vec<(
            u64,
            ActorCapability,
            tessera_protocol::CapabilityUseAction,
    ...
    refresh_result: Mutex<Result<Option<tessera_authority::authority::AgentIdentity>, String>>,
    ...
    principal_infos: Mutex<Vec<tessera_protocol::AgentPrincipalInfo>>,
    grant_infos: Mutex<Vec<tessera_protocol::AgentGrantInfo>>,
    management_log: Mutex<Vec<String>>,
    register_result: Mutex<Result<(String, String), String>>,
    resource_grants: Mutex<tessera_authority::authority::ResourceGrantRegistry>,
...
impl Handler for TestHandler {
    ...
    fn workspaces(&self) -> tessera_desktop::workspace::WorkspaceSnapshot {
        ...
        // the IPC plumbing tests; the model itself is exercised in tessera-desktop.
        ...
        tessera_desktop::workspace::WorkspaceSnapshot {
    ...
    fn enumerate_outputs(&self) -> Vec<tessera_protocol::OutputInfo> {
    ...
    fn transact(
        ...
        if let Some(expected) = expected_journal_seq
            && expected != before_seq
        {
            return Ok(tessera_protocol::TransactResult::PreconditionConflict {
                precondition: tessera_protocol::TransactPrecondition::JournalSeq,
                expected,
                actual: before_seq,
            });
        }
        ...
        {
            return Ok(tessera_protocol::TransactResult::PreconditionConflict {
                precondition: tessera_protocol::TransactPrecondition::InteractionDomainRevision,
                expected,
                actual: revision,
            });
        }
        ...
        for cmd in ops {
            ...
            after_seq += 1;
            results.push(tessera_protocol::TransactOpResult {
                seq: after_seq,
                effect: tessera_protocol::Effect::Applied,
            });
        }
        self.transact_seq.store(after_seq, Ordering::SeqCst);
        Ok(tessera_protocol::TransactResult::Committed {
            receipt: tessera_protocol::TransactReceipt {
                before_seq,
                after_seq,
                results,
            },
    ...
    fn audit_capability_use(
        ...
        if let tessera_protocol::Effect::Refused { reason } = effect {
    ...
    fn pick_app(
        ...
        Ok(tessera_protocol::AppPickResult::App {
    ...
    fn prompt_secret(
        ...
        Ok(tessera_protocol::SecretPromptResult::Secret {
    ...
    fn capture_window(
        ...
        Ok(tessera_ipc::CaptureWindowPayload {
            capture: tessera_protocol::WindowCapture {
                window,
                width: 2,
                height: 1,
                scale_milli: 1000,
    ...
    fn act_in_interaction_domain(
        ...
        if intent.observation != tessera_protocol::ObservationToken("b".repeat(64)) {
...
fn test_scopes() -> HashMap<String, Scope> {
    HashMap::from([
        (
            tessera_protocol::LOCAL_AGENT_ADMIN_SCOPE.into(),
            Scope {
                ops: Some(Vec::new()),
                ..Scope::default()
            },
        ...
        (
            ...
            Scope {
                interaction_domains: Some(vec![
                    tessera_authority::interaction_domain::HUMAN_INTERACTION_DOMAIN,
...
fn sample_outputs() -> Vec<tessera_protocol::OutputInfo> {
    vec![
        tessera_protocol::OutputInfo {
            ...
            geometry: Some(tessera_desktop::output::OutputGeometry {
                ...
                ..tessera_desktop::output::OutputGeometry::default()
            }),
            available_modes: Some(vec![tessera_desktop::output::OutputMode {
                width: 1920,
                height: 1080,
                refresh_mhz: 60_000,
            }]),
