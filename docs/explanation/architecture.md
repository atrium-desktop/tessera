...
Stable geometry, identities, and input values live in `tessera-types`.
Renderer-independent surface and stream descriptions live in `tessera-scene`.
...
and headless intent search live in `tessera-apps`.
